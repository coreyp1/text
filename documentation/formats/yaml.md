...
field, checked by value, and those marked `fail`, checked by refusal - **350
pass, 95.6%**. The other 38 assert an event stream the harness does not emit.
...
- **6 documents that should be refused are accepted.** Still the largest
  ...
  the next, and a sequence item may sit at an indentation of its own.
