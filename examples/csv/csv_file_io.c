...
static GTEXT_CSV_Status file_write_fn(
    void * user, const char * bytes, size_t len) {
  FILE * file = (FILE *)user;
  size_t written = fwrite(bytes, 1, len, file);
  return (written == len) ? GTEXT_CSV_OK : GTEXT_CSV_E_WRITE;
}
...
int main(void) {
  ...
  GTEXT_CSV_Parse_Options parse_opt = gtext_csv_parse_options_default();
  parse_opt.dialect.treat_first_row_as_header =
      true; // Enable header processing
  GTEXT_CSV_Error err = {0};

  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(csv_input, csv_len, &parse_opt, &err);
  if (!table) {
  ...
  gtext_csv_field_set(table, 0, 1, "31", 0); // Change Alice's age from 30 to 31
  ...
  gtext_csv_column_append(table, "Country", 0);
  printf("Added new column: Country\n");

  // Set values for the new column in existing rows
  gtext_csv_field_set(table, 0, 3, "USA", 0); // Alice
  gtext_csv_field_set(table, 1, 3, "USA", 0); // Bob
  gtext_csv_field_set(table, 2, 3, "USA", 0); // Charlie
  gtext_csv_field_set(table, 3, 3, "USA", 0); // David
  ...
  GTEXT_CSV_Status status =
      gtext_csv_write_table(&file_sink, &write_opt, table);
  fclose(output_file);
  ...
  GTEXT_CSV_Parse_Options read_parse_opt = gtext_csv_parse_options_default();
  read_parse_opt.dialect.treat_first_row_as_header =
      true; // Enable header processing
  GTEXT_CSV_Error read_err = {0};

  table =
      gtext_csv_parse_table(file_buffer, file_size, &read_parse_opt, &read_err);
  free(file_buffer);
  ...
  if (!table) {
    ...
    gtext_csv_error_free(&read_err);
    return 1;
  }

  printf(
      "Successfully read %zu rows from file\n\n", gtext_csv_row_count(table));
  ...
  GTEXT_CSV_Sink stdout_sink;
  if (gtext_csv_sink_buffer(&stdout_sink) != GTEXT_CSV_OK) {
    fprintf(stderr, "Failed to create buffer sink\n");
    gtext_csv_free_table(table);
    return 1;
  }

  GTEXT_CSV_Write_Options print_opt = gtext_csv_write_options_default();
  ...
  status = gtext_csv_write_table(&stdout_sink, &print_opt, table);
  if (status != GTEXT_CSV_OK) {
    fprintf(stderr, "Write error: %d\n", status);
    gtext_csv_sink_buffer_free(&stdout_sink);
    gtext_csv_free_table(table);
    return 1;
  }

  printf("CSV output (all fields quoted):\n");
  printf("%.*s\n", (int)gtext_csv_sink_buffer_size(&stdout_sink),
      gtext_csv_sink_buffer_data(&stdout_sink));
  ...
  printf("\n=== Example complete ===\n");
  printf("File %s has been created. You can inspect it to see the trailing "
         "newline.\n",
      filename);
