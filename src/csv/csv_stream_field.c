 ...
 * @file
 *
 * Field processing and unescaping for CSV streaming parser.
 *
...
GTEXT_CSV_Status csv_stream_complete_field(
  ...
  GTEXT_CSV_Status status = csv_stream_emit_field(stream, emit_record_end);
...
// This function encapsulates the common pattern of checking if we're at a chunk
// boundary and ensuring the field is buffered so it remains valid when the
// chunk is cleared.
GTEXT_CSV_Status csv_stream_handle_chunk_boundary(GTEXT_CSV_Stream * stream) {
  ...
  if (stream->field.is_buffered) {
    ...
    // Start buffering - ensure field is buffered (this will copy existing data
    // if needed)
    GTEXT_CSV_Status status = csv_field_buffer_ensure_buffered(&stream->field);
    if (status != GTEXT_CSV_OK) {
      return status;
    }
  }
...
GTEXT_CSV_Status csv_stream_emit_field(
  ...
  GTEXT_CSV_Status status = csv_stream_unescape_field(
      stream, field_data, actual_field_len, &unescaped_data, &unescaped_len);
  if (status != GTEXT_CSV_OK) {
    return status;
  }
...
GTEXT_CSV_Status csv_stream_validate_field_input(GTEXT_CSV_Stream * stream,
  ...
  if (!process_input) {
    return csv_stream_set_error(
        stream, GTEXT_CSV_E_INVALID, "process_input is NULL");
  }
  if (byte_pos > process_len) {
    return csv_stream_set_error(
        stream, GTEXT_CSV_E_INVALID, "Invalid byte position");
  }
  return GTEXT_CSV_OK;
}
...
bool csv_stream_can_use_in_situ(
  ...
  if (!stream->opts.in_situ_mode || !stream->original_input_buffer ||
      !field_start) {
    return false;
  }
...
// Scan ahead in unquoted field for special characters (delimiter, newline,
// quote) Returns the number of safe characters that can be processed in bulk,
// or 0 if a special character is found immediately.
// Sets *found_special to true if a special character was found, false
// otherwise. Sets *special_char to the character found (if found_special is
// true). Sets *special_pos to the position of the special character (if
// found_special is true).
  ...
  *found_special = false;
  ...
  while (pos < process_len) {
    ...
    if (c == '\n' || c == '\r') {
      ...
      csv_newline_type nl = csv_detect_newline(process_input, process_len,
          &pos_before, &stream->opts.dialect, &detect_error);
...
bool csv_stream_field_needs_unescape(
  ...
  // For quoted fields with doubled-quote escape, we always need to check for
  // doubled quotes even if needs_unescape is false (doubled quotes may have
  // been detected during parsing or may exist in the data from previous chunks)
  ...
  if (stream->field.is_quoted &&
    ...
    if (input_data == stream->field.buffer) {
      check_len = stream->field.buffer_used < input_len
          ? stream->field.buffer_used
          : input_len;
    }
...
GTEXT_CSV_Status csv_stream_unescape_field_no_unescape(
  ...
  // No unescaping needed, but we still need to ensure the data is in a stable
  // buffer If input_data points to field buffer, we can return it directly But
  // we must use buffer_used as the actual length, not input_len
  if (input_data == stream->field.buffer) {
    ...
    // Use the minimum of input_len and buffer_used to prevent reading past
    // buffer
  ...
  // Input is not in field buffer and not in original input - copy it to ensure
  // stability Safety check: ensure input_data is valid (not NULL)
  if (!input_data) {
  ...
  if (need_grow) {
    GTEXT_CSV_Status status =
        csv_field_buffer_grow(&stream->field, input_len + 1);
    if (status != GTEXT_CSV_OK) {
      return status;
    }
  }
...
GTEXT_CSV_Status csv_stream_unescape_field_with_unescape(
    GTEXT_CSV_Stream * stream, const char * input_data, size_t input_len,
    const char ** output_data, size_t * output_len) {
  // Check if input_data points to field buffer - if so, we need to handle
  // in-place operation
  bool input_is_field_buffer = (input_data == stream->field.buffer);
  const char * actual_input =
      input_data; // Local variable to track actual input pointer
  ...
  // CRITICAL: When unescaping in-place, we must not read past the actual valid
  // data Clamp input_len to buffer_used if input is in field buffer
  ...
  // This should never happen due to bounds checking in the loop above, but be
  // defensive
  if (out_idx > stream->field.buffer_size) {
    out_idx = stream->field.buffer_size;
  }
  // Additional check: if input was in field buffer, out_idx should not exceed
  // what we read
