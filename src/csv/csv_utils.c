 ...
 * @file
 *
 * Utility functions for CSV parsing: newline detection, BOM stripping, UTF-8
 * validation.
 *
...
// Sets *error_out to GTEXT_CSV_E_LIMIT if overflow occurs, GTEXT_CSV_OK
// otherwise
...
// Sets *error_out to GTEXT_CSV_E_LIMIT if overflow occurs, GTEXT_CSV_OK
// otherwise
csv_utf8_result csv_validate_utf8(const char * input, size_t input_len,
    csv_position * pos, bool validate, GTEXT_CSV_Status * error_out) {
  if (error_out) {
    *error_out = GTEXT_CSV_OK;
  }
  ...
  if (!validate) {
    ...
    if (pos) {
      ...
      if (input_len > (size_t)INT_MAX ||
          pos->column > INT_MAX - (int)input_len) {
...
// Returns GTEXT_CSV_OK on success (BOM stripped or no BOM found),
// GTEXT_CSV_E_LIMIT on overflow Sets *was_stripped to true if BOM was found and
...
GTEXT_INTERNAL_API GTEXT_CSV_Status csv_strip_bom(const char ** input,
  ...
  if (!input || !input_len || !*input || *input_len < 3) {
    return GTEXT_CSV_OK; // No BOM found (not an error)
  }

  if (!strip) {
    return GTEXT_CSV_OK; // Stripping disabled (not an error)
  }
  ...
  if ((unsigned char)(*input)[0] == 0xEF &&
    ...
    if (pos) {
      ...
      if (pos->offset > SIZE_MAX - 3) {
        return GTEXT_CSV_E_LIMIT; // Offset would overflow
      }
      if (pos->column > INT_MAX - 3) {
        return GTEXT_CSV_E_LIMIT; // Column would overflow
      }
      // All checks passed, perform all updates
      pos->offset += 3;
      pos->column += 3;
