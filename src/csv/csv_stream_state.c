/**
 * @file csv_stream_state.c
 * @brief State machine handlers for CSV streaming parser
 *
 * Handles all state transitions and character processing in the CSV parser state machine.
 */
...
text_csv_status csv_stream_process_start_of_field(
    ...
    csv_field_buffer_set_from_input(&stream->field, process_input + byte_pos, 0, false, byte_pos);
    ...
    if (c == stream->opts.dialect.delimiter) {
        ...
        if (stream->quote_in_quoted_at_chunk_boundary &&
            ...
            bool is_empty = stream->field.is_buffered ?
                (stream->field.buffer_used == 0) : (stream->field.length == 0);
            if (is_empty) {
                // Treat as doubled quote - ensure buffer is ready
                if (!stream->field.is_buffered) {
                    text_csv_status status = csv_field_buffer_grow(&stream->field, 2);
                    if (status != TEXT_CSV_OK) {
                        return status;
                    }
                    stream->field.buffer_used = 0;
...
text_csv_status csv_stream_process_escape_in_quoted(
    ...
    if (stream->field.is_buffered) {
        ...
        if (!stream->field.data) {
            csv_field_buffer_set_from_input(&stream->field, process_input + *offset - 1, 0, true, *offset - 1);
        }
...
text_csv_status csv_stream_process_chunk(
    ...
    if ((stream->state == CSV_STREAM_STATE_QUOTE_IN_QUOTED ||
        ...
        if (stream->field.is_buffered) {
            ...
            // Use ensure_buffered which will handle copying existing data correctly
