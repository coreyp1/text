/**
 * @file json_error.c
 * @brief Enhanced error reporting utilities for JSON module
 */
...
void text_json_error_free(text_json_error* err) {
...
text_json_status json_error_generate_context_snippet(
    ...
    // Check for overflow: if error_offset + context_after would overflow, use input_len
    if (error_offset <= SIZE_MAX - context_after && error_offset + context_after < input_len) {
    ...
    if (snippet_start > input_len || snippet_end > input_len || snippet_start > snippet_end) {
