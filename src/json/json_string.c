 ...
 * @file
 *
 * String and Unicode handling utilities for JSON module.
 *
  ...
  while (in_idx < input_len) {
    if (input[in_idx] == '\\') {
      ...
      if (esc_char == 'u') {
        ...
        uint32_t codepoint;
        size_t consumed = json_decode_unicode_escape(
            input + in_idx + 1, input_len - in_idx - 1, &codepoint);
        if (consumed == 0) {
          return GTEXT_JSON_E_BAD_UNICODE;
        }
        ...
        if (json_is_high_surrogate(codepoint)) {
          ...
          if (in_idx + 12 <= input_len && input[in_idx + 6] == '\\' &&
              input[in_idx + 7] == 'u') {
            uint32_t low_codepoint;
            size_t low_consumed = json_decode_unicode_escape(
                input + in_idx + 7, input_len - in_idx - 7, &low_codepoint);
            if (low_consumed > 0 && json_is_low_surrogate(low_codepoint)) {
              // Decode surrogate pair
              uint32_t full_codepoint;
              if (json_decode_surrogate_pair(
                      codepoint, low_codepoint, &full_codepoint)) {
      ...
      if (!allow_unescaped_controls && c < 0x20) {
        return GTEXT_JSON_E_BAD_TOKEN; // Unescaped control character
      }
