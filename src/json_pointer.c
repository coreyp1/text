...
static int json_pointer_parse_index(const char* str, size_t len, size_t* out_idx) {
    if (len == 0) {
        return 0;
    }
...
static text_json_value* json_pointer_evaluate(
    ...
    while (pos < len) {
        ...
        size_t decoded_len;
        text_json_status status = json_pointer_decode_token(
            ptr + token_start,
            token_len,
            decoded,
            token_len + 1,
        ...
        if (is_array_index) {
            // Try as array index
            if (current->type != TEXT_JSON_ARRAY) {
                free(decoded);
                return NULL;
            }
            ...
            // Try as object key
            if (current->type != TEXT_JSON_OBJECT) {
                free(decoded);
                return NULL;
            }

            // Search for the key in the object
            const text_json_value* found = text_json_object_get(
                (const text_json_value*)current,
                decoded,
                decoded_len
            );
...
const text_json_value* text_json_pointer_get(
    ...
    return json_pointer_evaluate((text_json_value*)root, ptr, len);
}

text_json_value* text_json_pointer_get_mut(
    text_json_value* root,
    const char* ptr,
    size_t len
) {
