...
TEST(CsvError, ContextSnippetBasic) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(invalid_input, invalid_len, &opts, &err);
...
TEST(CsvError, ContextSnippetCaretPosition) {
  ...
  GTEXT_CSV_Error err{};
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(input, input_len, &opts, &err);
...
TEST(CsvError, ContextSnippetStreamingParser) {
  ...
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  GTEXT_CSV_Error err{};
  auto callback = [](const GTEXT_CSV_Event * event,
...
TEST(CsvError, ContextSnippetDeepCopy) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(input, input_len, &opts, &err1);

  EXPECT_EQ(table, nullptr);
  EXPECT_EQ(err1.code, GTEXT_CSV_E_UNTERMINATED_QUOTE);

  // Context snippet may or may not be generated depending on input buffer
  // availability If it is generated, test deep copy
  if (err1.context_snippet && err1.context_snippet_len > 0) {
    ...
    EXPECT_EQ(memcmp(err1.context_snippet, err2.context_snippet,
                  err1.context_snippet_len),
        0);
...
TEST(CsvUtils, NewlineDetectionLF) {
  ...
  csv_newline_type result =
      csv_detect_newline(input, input_len, &pos, &dialect, &error);
...
TEST(CsvUtils, BOMNoStrip) {
  ...
  bool was_stripped = false;
  GTEXT_CSV_Status status =
      csv_strip_bom(&input, &input_len, &pos, false, &was_stripped);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvUtils, UTF8ValidationValidASCII) {
  ...
  GTEXT_CSV_Status error = GTEXT_CSV_OK;
  csv_utf8_result result =
      csv_validate_utf8(input, input_len, &pos, true, &error);
...
TEST(CsvUtils, UTF8ValidationDisabled) {
  ...
  GTEXT_CSV_Status error = GTEXT_CSV_OK;
  csv_utf8_result result =
      csv_validate_utf8(input, input_len, &pos, false, &error);
...
TEST(CsvStream, BasicParsing) {
  ...
  GTEXT_CSV_Event_cb callback = [](const GTEXT_CSV_Event * event,
                                    void * user_data) -> GTEXT_CSV_Status {
    auto * counts =
        (std::pair<size_t *, std::vector<std::string> *> *)user_data;
  ...
  GTEXT_CSV_Status status =
      gtext_csv_stream_feed(stream, input, input_len, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvStream, FieldSpanningManySmallChunks) {
  const char * full_field =
      "\"This is a field that will be split into many tiny chunks\"";
  ...
  for (size_t i = 0; i < full_len; ++i) {
    GTEXT_CSV_Status status =
        gtext_csv_stream_feed(stream, full_field + i, 1, nullptr);
    EXPECT_EQ(status, GTEXT_CSV_OK) << "Failed at byte " << i;
  }

  GTEXT_CSV_Status status =
      gtext_csv_stream_feed(stream, ",next\n", 6, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  EXPECT_EQ(fields.size(), 2u);
  EXPECT_EQ(
      fields[0], "This is a field that will be split into many tiny chunks");
  EXPECT_EQ(fields[1], "next");
}
...
TEST(CsvStream, LargeFieldSpanningChunks) {
  ...
  for (size_t i = 0; i < large_field.size(); i += chunk_size) {
    ...
    GTEXT_CSV_Status status = gtext_csv_stream_feed(
        stream, large_field.c_str() + i, chunk_len, nullptr);
...
// This verifies that delimiters at chunk boundaries don't cause incorrect field
// splitting
...
// Test case where delimiter is split across chunks after a quoted field with
// doubled quote Complete CSV: `1,"a b""c",d` where `""` is a doubled quote
// escape representing a literal quote So field2 should be `a b"c` (the doubled
// quote `""` becomes a single literal quote `"`) Chunk 1: `1,"a b""c"` -
// field1="1" complete, field2="a b""c" complete (doubled quote is within chunk)
//         The quote at end puts us in QUOTE_IN_QUOTED state, waiting to see if
//         it's closing or doubled But since the doubled quote is complete, we
//         know it's a closing quote, but delimiter is missing
// Chunk 2: `,d` - the delimiter at start completes field2, then field3="d"
// starts Should parse as: field1="1", field2="a b"c" (where "" is literal
// quote), field3="d"
TEST(CsvStream, DelimiterSplitAcrossChunksWithQuotedFields) {
  ...
  // Field2="a b""c" - the doubled quote is complete within chunk1, but ends
  // with quote Field2 is in QUOTE_IN_QUOTED state - cannot emit yet because we
  // need to see delimiter/newline
  ...
  // Expected result: field1="1", field2="a b"c" (where "" is literal quote),
  // field3="d"
  EXPECT_EQ(fields.size(), 3u);
...
// So field2 should be `a b"c` (the doubled quote `""` becomes a single literal
// quote `"`) Chunk 1: `1,"a b"` - field1="1" complete, field2 starts, ends with
// quote (enters QUOTE_IN_QUOTED state) Chunk 2: `"c",d` - the `"` at start
// should be recognized as second quote of doubled quote
//                     (if we're in QUOTE_IN_QUOTED state), making field2 = `a
//                     b"c`
TEST(CsvStream, DoubledQuoteSplitAcrossChunks) {
  // This tests the critical case: when a quote at end of chunk1 and quote at
  // start of chunk2 should be recognized as a doubled quote (escaped quote) if
  // we're in QUOTE_IN_QUOTED state Full CSV: `1,"a b""c",d` means: field1="1",
  // field2="a b"c" (where "" is literal quote), field3="d"
  ...
  // Expected result: field1="1", field2="a b"c" (where "" is literal quote),
  // field3="d" The doubled quote `""` should be recognized across chunks and
  // become a single literal quote
  EXPECT_EQ(fields.size(), 3u);
  EXPECT_EQ(fields[0], "1");
  EXPECT_EQ(fields[1], "a b\"c"); // The "" becomes a literal quote character
  EXPECT_EQ(fields[2], "d");
...
// Complete CSV: `1,"a b"\n2,"c"` where the newline comes immediately after the
// closing quote Chunk 1: `1,"a b"` - field1="1" complete, field2="a b" ends
// with quote (enters QUOTE_IN_QUOTED state)
//         The parser cannot know if this quote is:
//         - A closing quote (followed by newline/delimiter) -> field ends,
//         record ends
//         - First quote of doubled quote `""` (followed by another quote) ->
//         field continues So it must wait for the next chunk before emitting
//         field2
// Chunk 2: `\n2,"c"` - the newline at start should be recognized as ending the
// quoted field and record
...
TEST(CsvStream, NewlineSplitAcrossChunksAfterQuotedField) {
  ...
  EXPECT_EQ(status, GTEXT_CSV_OK);
  // After chunk1: field1="1" should be complete
  // Field2="a b" is in QUOTE_IN_QUOTED state - cannot emit yet because we don't
  // know if quote is closing or doubled

  status = gtext_csv_stream_feed(stream, chunk2, strlen(chunk2), nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  // When processing chunk2, we see the newline, which completes field2 and ends
  // record 1 Then record 2 starts with field1="2" and field2="c"
  ...
  EXPECT_EQ(
      record_boundaries[1], 4u); // Second record has 2 more fields (total 4)
}
...
TEST(CsvStream, CrlfNewlineSplitAcrossChunks) {
  ...
  // When CRLF is split, CR ends the first record, LF starts a new record
  // (empty), then field2 is in the next record So we get: record1: field1,
  // record2: (empty), record3: field2
...
TEST(CsvStream, QuoteAtBoundaryFollowedByInvalidCharacter) {
  ...
  // Should fail with invalid quote usage - quote must be followed by delimiter,
  // newline, or another quote
  EXPECT_NE(status, GTEXT_CSV_OK);
...
TEST(CsvStream, VerySmallChunksWithComplexSequences) {
  ...
  for (size_t i = 0; i < num_chunks; ++i) {
    GTEXT_CSV_Status status =
        gtext_csv_stream_feed(stream, chunks[i], strlen(chunks[i]), nullptr);
...
// Note: When a quote appears after an unquoted field, it typically starts a new
// quoted field This test verifies the parser handles this transition correctly
...
TEST(CsvTable, HeaderIndexNext) {
  // Test gtext_csv_header_index_next() with duplicate headers created via
  // append This avoids reindexing complexity
  const char * headers[] = {"col"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 1);
  ASSERT_NE(table, nullptr);

  // Add a row
  const char * fields[] = {"val1"};
  EXPECT_EQ(gtext_csv_row_append(table, fields, nullptr, 1, nullptr), GTEXT_CSV_OK);

  // Append two more columns with the same name (duplicates allowed by default)
  EXPECT_EQ(
      gtext_csv_column_append(table, "col", 0), GTEXT_CSV_OK); // col at index 1
  EXPECT_EQ(
      gtext_csv_column_append(table, "col", 0), GTEXT_CSV_OK); // col at index 2
  ...
  if (first_idx < 2) {
    size_t next_idx;
    EXPECT_EQ(gtext_csv_header_index_next(table, "col", first_idx, &next_idx),
        GTEXT_CSV_OK);
    ...
    if (first_idx == 0) {
      size_t next2_idx;
      EXPECT_EQ(gtext_csv_header_index_next(table, "col", next_idx, &next2_idx),
          GTEXT_CSV_OK);
      ...
      size_t next3_idx;
      EXPECT_EQ(
          gtext_csv_header_index_next(table, "col", next2_idx, &next3_idx),
          GTEXT_CSV_E_INVALID);
    }
  }
  ...
  size_t test_idx;
  if (gtext_csv_header_index(table, "col", &test_idx) == GTEXT_CSV_OK &&
      test_idx == 2) {
    size_t no_next;
    EXPECT_EQ(gtext_csv_header_index_next(table, "col", 2, &no_next),
        GTEXT_CSV_E_INVALID);
  }
...
TEST(CsvTable, HeaderIndexNextUniqueHeader) {
  ...
  size_t next_idx;
  EXPECT_EQ(gtext_csv_header_index_next(table, "name", idx, &next_idx),
      GTEXT_CSV_E_INVALID);
...
TEST(CsvTable, HeaderIndexNextInvalidCurrentIdx) {
  ...
  size_t next_idx;
  EXPECT_EQ(gtext_csv_header_index_next(table, "status", 100, &next_idx),
      GTEXT_CSV_E_INVALID);

  // Try with current_idx that doesn't match the header name
  EXPECT_EQ(gtext_csv_header_index_next(table, "status", 3, &next_idx),
      GTEXT_CSV_E_INVALID); // Index 3 is "name"
...
TEST(CsvTable, HeaderIndexNextNonExistentHeader) {
  ...
  size_t next_idx;
  EXPECT_EQ(gtext_csv_header_index_next(table, "nonexistent", 0, &next_idx),
      GTEXT_CSV_E_INVALID);
...
TEST(CsvTable, HeaderIndexNextCompleteIteration) {
  // Test complete iteration: gtext_csv_header_index() then repeated
  // gtext_csv_header_index_next() Use append to avoid reindexing complexity
  const char * headers[] = {"col"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 1);
  ASSERT_NE(table, nullptr);
  ...
  while (iterations < max_iterations &&
      gtext_csv_header_index_next(table, "col", current, &next) ==
          GTEXT_CSV_OK) {
...
TEST(CsvTable, HeaderIndexNextMutationOperations) {
  // Test header_index_next with duplicate headers created via mutation
  // operations
  const char * headers[] = {"col1", "col2", "col3"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 3);
  ASSERT_NE(table, nullptr);

  // Add a row
  const char * fields[] = {"val1", "val2", "val3"};
  EXPECT_EQ(gtext_csv_row_append(table, fields, nullptr, 3, nullptr), GTEXT_CSV_OK);
  ...
  if (idx == 0u) {
    ...
    EXPECT_EQ(gtext_csv_header_index_next(table, "col1", idx, &next_idx),
        GTEXT_CSV_OK);
    EXPECT_EQ(next_idx, 3u); // Second match (appended column)
    // No more matches after 3
    EXPECT_EQ(gtext_csv_header_index_next(table, "col1", next_idx, &next_idx),
        GTEXT_CSV_E_INVALID);
  }
  else {
...
TEST(CsvTable, DuplicateColumnNamesExplicitErrorMode) {
  ...
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  opts.dialect.treat_first_row_as_header = true;
  opts.dialect.header_dup_mode =
      GTEXT_CSV_DUPCOL_ERROR; // Explicitly set error mode
  GTEXT_CSV_Error err{};
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(input, input_len, &opts, &err);
...
TEST(CsvSink, CallbackSink) {
  ...
  auto write_callback = [](void * user, const char * bytes,
                            size_t len) -> GTEXT_CSV_Status {
...
TEST(CsvSink, FixedBuffer) {
  ...
  GTEXT_CSV_Sink sink;
  GTEXT_CSV_Status status =
      gtext_csv_sink_fixed_buffer(&sink, buffer, sizeof(buffer));
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvSink, FixedBufferTruncation) {
  ...
  const char * large_data =
      "This is a very long string that will exceed the buffer size";
...
TEST(CsvSink, FixedBufferInvalidParams) {
  ...
  // Test NULL sink
  GTEXT_CSV_Status status =
      gtext_csv_sink_fixed_buffer(nullptr, (char *)"test", 4);
...
TEST(CsvWriter, FieldRoundTripSimple) {
  ...
  GTEXT_CSV_Stream * stream =
      gtext_csv_stream_new(&parse_opts, callback, &field_value);
...
TEST(CsvTableWrite, SimpleTable) {
  ...
  const char * input = "a,b,c\n1,2,3\n4,5,6";
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(input, strlen(input), nullptr, nullptr);
...
TEST(CsvTableWrite, RoundTripSimple) {
  ...
  // Parse the output
  GTEXT_CSV_Table * table2 =
      gtext_csv_parse_table(output, output_len, nullptr, nullptr);
  ...
  for (size_t row = 0; row < gtext_csv_row_count(table1); row++) {
    EXPECT_EQ(
        gtext_csv_col_count(table1, row), gtext_csv_col_count(table2, row));
...
TEST(CsvTableWrite, WithHeader) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(input, strlen(input), &parse_opts, nullptr);
  EXPECT_NE(table, nullptr);
...
TEST(CsvTableWrite, RoundTripWithHeader) {
  ...
  GTEXT_CSV_Table * table2 =
      gtext_csv_parse_table(output, output_len, &parse_opts, nullptr);
  EXPECT_NE(table2, nullptr);
...
// ============================================================================
// Write Trimming Tests
// ============================================================================

TEST(CsvWriteTrimming, NoTrimmingDefault) {
  // Create table with trailing empty fields
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Enable irregular rows to allow different field counts
  EXPECT_EQ(gtext_csv_set_allow_irregular_rows(table, true), GTEXT_CSV_OK);

  // Add row with trailing empty fields: "a,b,," (4 fields, last 2 empty)
  const char * fields1[] = {"a", "b", "", ""};
  EXPECT_EQ(gtext_csv_row_append(table, fields1, nullptr, 4, nullptr),
            GTEXT_CSV_OK);

  // Write without trimming (default)
  GTEXT_CSV_Sink sink;
  GTEXT_CSV_Status status = gtext_csv_sink_buffer(&sink);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  GTEXT_CSV_Write_Options opts = gtext_csv_write_options_default();
  // trim_trailing_empty_fields defaults to false
  EXPECT_FALSE(opts.trim_trailing_empty_fields);

  status = gtext_csv_write_table(&sink, &opts, table);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Should include trailing empty fields (quoted empty fields)
  EXPECT_TRUE(output_str.find("a,b,\"\",\"\"") != std::string::npos ||
              output_str.find("a,b,,") != std::string::npos);

  gtext_csv_free_table(table);
  gtext_csv_sink_buffer_free(&sink);
}

TEST(CsvWriteTrimming, TrimmingTrailingEmptyFields) {
  // Create table with trailing empty fields
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Add row: "a,b,," (4 fields, last 2 empty)
  const char * fields2[] = {"a", "b", "", ""};
  EXPECT_EQ(gtext_csv_row_append(table, fields2, nullptr, 4, nullptr),
            GTEXT_CSV_OK);
  ...
  // Should trim trailing empty fields - output should be "a,b"
  EXPECT_TRUE(output_str.find("a,b") != std::string::npos);
...
TEST(CsvWriteTrimming, NoTrailingEmptyFields) {
  // Create table without trailing empty fields
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Add row: "a,b,c" (all fields non-empty)
  const char * fields3[] = {"a", "b", "c"};
  EXPECT_EQ(gtext_csv_row_append(table, fields3, nullptr, 3, nullptr),
            GTEXT_CSV_OK);
...
TEST(CsvWriteTrimming, EmptyFieldInMiddle) {
  // Create table with empty field in middle (not trailing)
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Add row: "a,,b" (empty field in middle, not trailing)
  const char * fields4[] = {"a", "", "b"};
  EXPECT_EQ(gtext_csv_row_append(table, fields4, nullptr, 3, nullptr),
            GTEXT_CSV_OK);
  ...
  // Should write all fields including empty in middle: "a,,b" or "a,\"\",b"
  EXPECT_TRUE(output_str.find("a,") != std::string::npos);
  EXPECT_TRUE(output_str.find(",b") != std::string::npos);

  gtext_csv_free_table(table);
  gtext_csv_sink_buffer_free(&sink);
}

TEST(CsvWriteTrimming, AllFieldsEmpty) {
  // Create table with all fields empty
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Add row with all empty fields: ",,"
  const char * fields5[] = {"", "", ""};
  EXPECT_EQ(gtext_csv_row_append(table, fields5, nullptr, 3, nullptr),
            GTEXT_CSV_OK);
  ...
  // All fields empty - should output empty line (just newline)
  // The line should exist but have no fields
  EXPECT_TRUE(output_str.find("\n") != std::string::npos);

  gtext_csv_free_table(table);
  gtext_csv_sink_buffer_free(&sink);
}

TEST(CsvWriteTrimming, WithHeaders) {
  // Create table with headers and trailing empty fields
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Set header row with trailing empty: "col1,col2,,"
  const char * header_fields[] = {"col1", "col2", "", ""};
  EXPECT_EQ(gtext_csv_row_append(table, header_fields, nullptr, 4, nullptr),
            GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_set_header_row(table, true), GTEXT_CSV_OK);

  // Add data row with trailing empty: "a,b,,"
  const char * data_fields[] = {"a", "b", "", ""};
  EXPECT_EQ(gtext_csv_row_append(table, data_fields, nullptr, 4, nullptr),
            GTEXT_CSV_OK);
  ...
  // Both header and data rows should be trimmed
  EXPECT_TRUE(output_str.find("col1,col2") != std::string::npos);
  EXPECT_TRUE(output_str.find("a,b") != std::string::npos);
  // Should NOT contain trailing commas
  EXPECT_TRUE(output_str.find("col1,col2,") == std::string::npos);
  EXPECT_TRUE(output_str.find("a,b,") == std::string::npos);
...
TEST(CsvWriteTrimming, RoundTripLossOfTrailingEmpties) {
  // Parse CSV with trailing empty fields
  const char * input = "a,b,,\nc,d,,\n";
  GTEXT_CSV_Parse_Options parse_opts = gtext_csv_parse_options_default();
  parse_opts.dialect.treat_first_row_as_header = false;

  GTEXT_CSV_Table * table1 =
      gtext_csv_parse_table(input, strlen(input), &parse_opts, nullptr);
  ASSERT_NE(table1, nullptr);

  // Enable irregular rows (parsed table may already have it, but ensure)
  EXPECT_EQ(gtext_csv_set_allow_irregular_rows(table1, true), GTEXT_CSV_OK);
  ...
  status = gtext_csv_write_table(&sink, &write_opts, table1);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Verify trailing empty fields were lost (round-trip limitation)
  // Original had 4 fields per row, output should have 2 fields per row
  EXPECT_EQ(gtext_csv_col_count(table2, 0), 2u);
  EXPECT_EQ(gtext_csv_col_count(table2, 1), 2u);

  // Verify content matches (non-empty fields preserved)
  size_t len;
  const char * field = gtext_csv_field(table2, 0, 0, &len);
  EXPECT_EQ(std::string(field, len), "a");
  field = gtext_csv_field(table2, 0, 1, &len);
  EXPECT_EQ(std::string(field, len), "b");
  field = gtext_csv_field(table2, 1, 0, &len);
  EXPECT_EQ(std::string(field, len), "c");
  field = gtext_csv_field(table2, 1, 1, &len);
  EXPECT_EQ(std::string(field, len), "d");
...
TEST(CsvTableInSitu, BasicInSitu) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(input, strlen(input), &opts, nullptr);
  EXPECT_NE(table, nullptr);
  EXPECT_EQ(gtext_csv_row_count(table), 1u);
  EXPECT_EQ(gtext_csv_col_count(table, 0), 3u);

  // Verify content is correct (in-situ mode may or may not be used depending on
  // implementation)
  size_t len;
...
TEST(CsvTableInSitu, QuotedFieldsFallback) {
  // Quoted fields with doubled quotes need unescaping - should fall back to
  // copy
  const char * input = "a,\"b\"\"c\",d";
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  opts.in_situ_mode = true;
  ...
  // Middle field (quoted with doubled quotes) needs unescaping - should be
  // copied
  const char * field1 = gtext_csv_field(table, 0, 1, &len);
  EXPECT_NE(field1, nullptr);
...
static void test_valid_csv_file(const std::string & filepath) {
  ...
  GTEXT_CSV_Error err{};
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(content.c_str(), content.size(), &opts, &err);
  EXPECT_NE(table, nullptr)
      << "Failed to parse valid CSV from: " << filepath
...
static void test_valid_csv_stream(const std::string & filepath) {
  ...
  GTEXT_CSV_Error err{};
  GTEXT_CSV_Status status =
      gtext_csv_stream_feed(stream, content.c_str(), content.size(), &err);
  EXPECT_EQ(status, GTEXT_CSV_OK) << "Failed to feed stream from: " << filepath;

  if (status == GTEXT_CSV_OK) {
    status = gtext_csv_stream_finish(stream, &err);
    EXPECT_EQ(status, GTEXT_CSV_OK)
        << "Failed to finish stream from: " << filepath;
  }
...
static void test_invalid_csv_file(const std::string & filepath) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(content.c_str(), content.size(), &opts, &err);
  EXPECT_EQ(table, nullptr)
      << "Should have failed to parse invalid CSV from: " << filepath;
...
static void test_round_trip(const std::string & filepath) {
  ...
  GTEXT_CSV_Error err{};
  // Parse original
  GTEXT_CSV_Table * original =
      gtext_csv_parse_table(content.c_str(), content.size(), &parse_opts, &err);
  ...
  // Parse again
  GTEXT_CSV_Table * reparsed =
      gtext_csv_parse_table(output, output_len, &parse_opts, &err);
  ...
  if (original && reparsed) {
    ...
    size_t min_rows =
        std::min(gtext_csv_row_count(original), gtext_csv_row_count(reparsed));
    for (size_t row = 0; row < min_rows; row++) {
      EXPECT_EQ(gtext_csv_col_count(original, row),
          gtext_csv_col_count(reparsed, row))
          << "Round-trip col count mismatch at row " << row
          << " for: " << filepath;

      size_t min_cols = std::min(gtext_csv_col_count(original, row),
          gtext_csv_col_count(reparsed, row));
      for (size_t col = 0; col < min_cols; col++) {
        ...
        EXPECT_EQ(len1, len2) << "Round-trip field length mismatch at row "
                              << row << ", col " << col << " for: " << filepath;
        if (len1 == len2) {
          EXPECT_EQ(memcmp(field1, field2, len1), 0)
              << "Round-trip field content mismatch at row " << row << ", col "
              << col << " for: " << filepath;
        }
      }
    }
...
TEST(TestCorpus, EdgeCases) {
  ...
  if (!bom_content.empty()) {
    ...
    GTEXT_CSV_Error err{};
    GTEXT_CSV_Table * table = gtext_csv_parse_table(
        bom_content.c_str(), bom_content.size(), &opts, &err);
    EXPECT_NE(table, nullptr);
    if (table) {
      gtext_csv_free_table(table);
    }
...
TEST(TestCorpus, EdgeCasesNewlines) {
  ...
  if (!crlf_content.empty()) {
    ...
    GTEXT_CSV_Error err{};
    GTEXT_CSV_Table * table = gtext_csv_parse_table(
        crlf_content.c_str(), crlf_content.size(), &opts, &err);
    EXPECT_NE(table, nullptr);
    if (table) {
      gtext_csv_free_table(table);
    }
  ...
  if (!cr_content.empty()) {
    ...
    GTEXT_CSV_Error err{};
    GTEXT_CSV_Table * table = gtext_csv_parse_table(
        cr_content.c_str(), cr_content.size(), &opts, &err);
  ...
  if (!mixed_content.empty()) {
    ...
    GTEXT_CSV_Error err{};
    GTEXT_CSV_Table * table = gtext_csv_parse_table(
        mixed_content.c_str(), mixed_content.size(), &opts, &err);
    EXPECT_NE(table, nullptr);
    if (table) {
      gtext_csv_free_table(table);
    }
...
TEST(TestCorpus, UnequalColumnCounts) {
  ...
  // With default options, first row is NOT treated as header, so all 5 rows are
  // data rows Row 0: "name,age,city" - 3 columns (header row, but treated as
  // data) Row 1: "Alice,30" - 2 columns (missing city) Row 2: "Bob,25,LA,extra"
  // - 4 columns (extra field) Row 3: "Charlie" - 1 column (missing age and
  // city) Row 4: "Diana,28,NYC,extra1,extra2" - 5 columns (two extra fields)
...
TEST(TestCorpus, ConsecutiveEmptyFields) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(content.c_str(), content.size(), &opts, &err);
  ASSERT_NE(table, nullptr)
      << "Failed to parse CSV with consecutive empty fields";

  // With default options, first row is NOT treated as header, so all rows are
  // data rows Row 0: "a,b,c" - 3 columns Row 1: "foo",,,,,"bar" - 6 columns (4
  // empty fields in middle) Row 2: "start",,,"middle",,"end" - 6 columns (empty
  // fields at positions 1, 2, 4) Row 3: ,,,"only_last" - 4 columns (empty
  // fields at positions 0, 1, 2) Row 4: "only_first",,, - 4 columns (empty
  // fields at positions 1, 2, 3)
  EXPECT_EQ(gtext_csv_row_count(table), 5u);
...
TEST(TestCorpus, InvalidCases) {
  ...
  std::string invalid_escape_content =
      read_file(base_dir + "/invalid-escape.csv");
  if (!invalid_escape_content.empty()) {
    GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
    opts.dialect.escape = GTEXT_CSV_ESCAPE_BACKSLASH;
    GTEXT_CSV_Error err{};
    GTEXT_CSV_Table * table =
        gtext_csv_parse_table(invalid_escape_content.c_str(),
            invalid_escape_content.size(), &opts, &err);
    EXPECT_EQ(table, nullptr)
        << "Should have failed to parse invalid escape sequence";
    if (table) {
      gtext_csv_free_table(table);
    }
    gtext_csv_error_free(&err);
...
TEST(TestCorpus, MilestoneDialectMatrix) {
  ...
  // Test TSV dialect
  {
    ...
    GTEXT_CSV_Error err{};
    GTEXT_CSV_Table * table = gtext_csv_parse_table(
        content.c_str(), content.size(), &parse_opts, &err);
    ASSERT_NE(table, nullptr);
  ...
  {
    std::string content =
        read_file(base_dir + "/dialects/backslash-escape/basic.csv");
    ASSERT_FALSE(content.empty());
...
TEST(CsvMutation, RowAppendFirstRow) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(table, fields, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_count(table), 1u);
  EXPECT_EQ(table->column_count, 3u);
...
TEST(CsvMutation, RowAppendMultipleRows) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  const char * row2[] = {"d", "e", "f"};
  status = gtext_csv_row_append(table, row2, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowAppendFieldCountMismatch) {
  ...
  const char * row2[] = {"d", "e"};
  status = gtext_csv_row_append(table, row2, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
  EXPECT_EQ(gtext_csv_row_count(table), 1u);
...
TEST(CsvMutation, RowAppendNullTable) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(nullptr, fields, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
}
...
TEST(CsvMutation, RowAppendNullFields) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(table, nullptr, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
...
TEST(CsvMutation, RowAppendZeroFieldCount) {
  ...
  const char * fields[] = {"a"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, fields, nullptr, 0, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
...
TEST(CsvMutation, RowAppendWithExplicitLengths) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(table, fields, lengths, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowAppendWithNullBytes) {
  ...
  //
  // This is a single field containing 3 bytes: 'a', '\0', 'b' (not three
  // separate strings). The null byte is part of the field data, not a string
  // terminator.
  //
  // Note: Null bytes are unusual in CSV (which is typically text-based), but
  // the implementation correctly preserves binary data. This test verifies:
  ...
  //
  // In practice, null bytes in CSV may not be compatible with all CSV tools,
  // but our implementation handles them correctly for completeness.
  const char field_data[] = {
      'a', '\0', 'b'}; // One field: 3 bytes including null
  const char * fields[] = {field_data};
  size_t lengths[] = {3}; // Explicit length tells us to copy all 3 bytes
  GTEXT_CSV_Status status = gtext_csv_row_append(table, fields, lengths, 1, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Verify that fields[0] is not the same as field (which is a pointer to the 3
  // bytes in the arena)
...
TEST(CsvMutation, RowAppendCapacityGrowth) {
  ...
  for (size_t i = 0; i < 20; i++) {
    ...
    const char * fields[] = {field_data};
    GTEXT_CSV_Status status = gtext_csv_row_append(table, fields, nullptr, 1, nullptr);
    EXPECT_EQ(status, GTEXT_CSV_OK);
  }
...
TEST(CsvMutation, RowInsertAtBeginning) {
  ...
  status = gtext_csv_row_insert(table, 0, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowInsertInMiddle) {
  ...
  status = gtext_csv_row_append(table, row3, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Insert in middle (idx = 1)
  const char * new_row[] = {"x", "y", "z"};
  status = gtext_csv_row_insert(table, 1, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowInsertAtEnd) {
  ...
  const char * new_row[] = {"x", "y", "z"};
  status = gtext_csv_row_insert(table, 2, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_count(table), 3u);
...
TEST(CsvMutation, RowInsertFieldCountValidation) {
  ...
  status = gtext_csv_row_insert(table, 0, new_row, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
  EXPECT_EQ(gtext_csv_row_count(table), 1u);
...
TEST(CsvMutation, RowInsertCapacityGrowth) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_insert(table, 5, new_row, nullptr, 1, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowInsertNullTable) {
  const char * fields[] = {"a", "b"};
  GTEXT_CSV_Status status =
      gtext_csv_row_insert(nullptr, 0, fields, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
}
...
TEST(CsvMutation, RowInsertNullFields) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_insert(table, 0, nullptr, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
...
TEST(CsvMutation, RowInsertEmptyTable) {
  ...
  const char * fields[] = {"a", "b", "c"};
  GTEXT_CSV_Status status = gtext_csv_row_insert(table, 0, fields, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_count(table), 1u);
  EXPECT_EQ(table->column_count, 3u);
...
TEST(CsvMutation, RowInsertWithExplicitLengths) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, lengths1, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  const char * new_row[] = {"xyz", "uvw", "rst"};
  size_t lengths2[] = {3, 3, 3};
  status = gtext_csv_row_insert(table, 0, new_row, lengths2, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowInsertWithNullBytes) {
  ...
  const char * row1[] = {"a", "b"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  size_t lengths[] = {3, 6};
  status = gtext_csv_row_insert(table, 0, fields, lengths, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowInsertFieldDataCopied) {
  ...
  status = gtext_csv_row_insert(table, 0, fields, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowRemoveHeaderRow) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(csv_data, strlen(csv_data), &opts, nullptr);
  ASSERT_NE(table, nullptr);
  EXPECT_EQ(gtext_csv_row_count(table), 2u); // 2 data rows

  // Header row is protected - it's not accessible via external API (row_idx is
  // 0-based for data rows only) row_idx=0 refers to the first data row (Alice),
  // not the header

  // Can remove data rows
  GTEXT_CSV_Status status =
      gtext_csv_row_remove(table, 0); // Remove first data row (Alice)
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_count(table), 1u);
...
TEST(CsvMutation, RowRemoveRowShifting) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row0, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  const char * row1[] = {"row1", "col1", "col2"};
  status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowSetAtBeginning) {
  ...
  // Replace first row
  const char * new_row[] = {"x", "y", "z"};
  status = gtext_csv_row_set(table, 0, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowSetInMiddle) {
  ...
  // Replace middle row
  const char * new_row[] = {"x", "y", "z"};
  status = gtext_csv_row_set(table, 1, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowSetAtEnd) {
  ...
  // Replace last row
  const char * new_row[] = {"x", "y", "z"};
  status = gtext_csv_row_set(table, 2, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowSetFieldCountValidation) {
  ...
  const char * new_row[] = {"x", "y"};
  status = gtext_csv_row_set(table, 0, new_row, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
...
TEST(CsvMutation, RowSetWithHeaderRow) {
  ...
  GTEXT_CSV_Error err{};
  GTEXT_CSV_Table * table =
      gtext_csv_parse_table(csv_data, strlen(csv_data), &opts, &err);
  ASSERT_NE(table, nullptr);
  ...
  GTEXT_CSV_Status status = gtext_csv_row_set(table, 0, new_row, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, RowSetNullParameters) {
  ...
  // Test NULL table
  const char * new_row[] = {"x", "y", "z"};
  status = gtext_csv_row_set(nullptr, 0, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);

  // Test NULL fields
  status = gtext_csv_row_set(table, 0, nullptr, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
...
TEST(CsvMutation, RowSetWithExplicitLengths) {
  ...
  const char * new_row[] = {"xyz", "uvw", "rst"};
  size_t lengths[] = {3, 3, 3};
  status = gtext_csv_row_set(table, 0, new_row, lengths, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
// Irregular Rows Support Tests
TEST(CsvIrregularRows, FlagDefaultValue) {
  // Test default value is false for new tables
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  EXPECT_FALSE(gtext_csv_get_allow_irregular_rows(table));
  gtext_csv_free_table(table);

  // Test default value is false for parsed tables
  const char * csv_data = "a,b,c\nd,e,f";
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  GTEXT_CSV_Error err{};
  table = gtext_csv_parse_table(csv_data, strlen(csv_data), &opts, &err);
  ASSERT_NE(table, nullptr);
  EXPECT_FALSE(gtext_csv_get_allow_irregular_rows(table));
  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, FlagGetterSetter) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Set to true
  GTEXT_CSV_Status status = gtext_csv_set_allow_irregular_rows(table, true);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_TRUE(gtext_csv_get_allow_irregular_rows(table));

  // Set to false
  status = gtext_csv_set_allow_irregular_rows(table, false);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_FALSE(gtext_csv_get_allow_irregular_rows(table));

  // Test NULL table handling
  EXPECT_FALSE(gtext_csv_get_allow_irregular_rows(nullptr));
  status = gtext_csv_set_allow_irregular_rows(nullptr, true);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, RowAppendStrictMode) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // First row sets column count
  const char * row1[] = {"a", "b", "c"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 3u);

  // Append with wrong field count should fail in strict mode
  const char * row2[] = {"d", "e"};
  status = gtext_csv_row_append(table, row2, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
  EXPECT_EQ(gtext_csv_row_count(table), 1u);
...
TEST(CsvIrregularRows, RowAppendIrregularMode) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Append row with more fields - should succeed and update column_count
  const char * row2[] = {"c", "d", "e", "f"};
  status = gtext_csv_row_append(table, row2, nullptr, 4, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Append row with fewer fields - should succeed, column_count stays at max
  const char * row3[] = {"g"};
  status = gtext_csv_row_append(table, row3, nullptr, 1, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 4u); // Max remains
  EXPECT_EQ(gtext_csv_col_count(table, 2), 1u);

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, RowInsertIrregularMode) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Add initial rows
  const char * row1[] = {"a", "b"};
  gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  const char * row2[] = {"c", "d", "e"};
  gtext_csv_row_append(table, row2, nullptr, 3, nullptr);
  EXPECT_EQ(table->column_count, 3u);

  // Insert row with different field count
  const char * row3[] = {"f", "g", "h", "i", "j"};
  GTEXT_CSV_Status status = gtext_csv_row_insert(table, 1, row3, nullptr, 5, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 5u); // Updated to max
  EXPECT_EQ(gtext_csv_col_count(table, 1), 5u);

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, RowSetIrregularMode) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Add initial rows
  const char * row1[] = {"a", "b", "c"};
  gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  const char * row2[] = {"d", "e"};
  gtext_csv_row_append(table, row2, nullptr, 2, nullptr);
  EXPECT_EQ(table->column_count, 3u);

  // Replace row with more fields
  const char * new_row[] = {"x", "y", "z", "w"};
  GTEXT_CSV_Status status = gtext_csv_row_set(table, 1, new_row, nullptr, 4, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 4u); // Updated to max
  EXPECT_EQ(gtext_csv_col_count(table, 1), 4u);

  // Replace row with fewer fields
  const char * new_row2[] = {"p"};
  status = gtext_csv_row_set(table, 0, new_row2, nullptr, 1, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 4u); // Max remains
  EXPECT_EQ(gtext_csv_col_count(table, 0), 1u);

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, ColumnCountTracking) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  gtext_csv_set_allow_irregular_rows(table, true);

  // Add rows with varying field counts
  const char * row1[] = {"a"};
  gtext_csv_row_append(table, row1, nullptr, 1, nullptr);
  EXPECT_EQ(table->column_count, 1u);

  const char * row2[] = {"b", "c", "d"};
  gtext_csv_row_append(table, row2, nullptr, 3, nullptr);
  EXPECT_EQ(table->column_count, 3u);

  const char * row3[] = {"e", "f"};
  gtext_csv_row_append(table, row3, nullptr, 2, nullptr);
  EXPECT_EQ(table->column_count, 3u); // Max remains

  const char * row4[] = {"g", "h", "i", "j", "k"};
  gtext_csv_row_append(table, row4, nullptr, 5, nullptr);
  EXPECT_EQ(table->column_count, 5u); // Updated to new max

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, EmptyTableFirstRow) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Irregular mode
  table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  gtext_csv_set_allow_irregular_rows(table, true);

  status = gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 2u);
...
TEST(CsvIrregularRows, WithHeaders) {
  const char * headers[] = {"col1", "col2", "col3"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 3);
  ASSERT_NE(table, nullptr);
  ...
  // Append rows with varying field counts
  const char * row1[] = {"a", "b"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  status = gtext_csv_row_append(table, row2, nullptr, 4, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 4u); // Max across all rows

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, FlagPersistence) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
...
TEST(CsvIrregularRows, ParsedIrregularThenMutate) {
  // Parse CSV with irregular rows
  const char * csv_data = "a,b,c\nd,e\nf,g,h,i";
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  GTEXT_CSV_Error err{};
  GTEXT_CSV_Table * table =
  ...
  // Enable irregular rows and mutate
  gtext_csv_set_allow_irregular_rows(table, true);

  const char * new_row[] = {"x", "y", "z", "w", "v"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, new_row, nullptr, 5, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 5u); // Max updated

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, RowRemoveRecalculatesColumnCount) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Add rows with varying field counts
  const char * row1[] = {"a", "b"};           // 2 fields
  const char * row2[] = {"c", "d", "e"};      // 3 fields (max)
  const char * row3[] = {"f", "g"};           // 2 fields

  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  status = gtext_csv_row_append(table, row2, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  status = gtext_csv_row_append(table, row3, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify column_count is max (3)
  EXPECT_EQ(table->column_count, 3u);

  // Remove row with maximum field_count (row2 at index 1)
  status = gtext_csv_row_remove(table, 1);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify column_count is recalculated to new max (2)
  EXPECT_EQ(table->column_count, 2u);

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, RowRemoveNonMaxDoesNotRecalculate) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Remove row with non-maximum field_count (row1 at index 0)
  status = gtext_csv_row_remove(table, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify column_count unchanged (still 3, max row still present)
  EXPECT_EQ(table->column_count, 3u);

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, RowRemoveLastRow) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Verify column_count is 0 (empty table)
  EXPECT_EQ(table->column_count, 0u);
  EXPECT_EQ(gtext_csv_row_count(table), 0u);
...
TEST(CsvIrregularRows, RowRemoveWithHeaders) {
  const char * headers[] = {"col1", "col2", "col3"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 3);
  ASSERT_NE(table, nullptr);
  ...
  // Add rows with varying field counts
  const char * row1[] = {"a", "b"};           // 2 fields
  const char * row2[] = {"c", "d", "e", "f"}; // 4 fields (max)
  const char * row3[] = {"g", "h"};           // 2 fields
  ...
  // Verify column_count is max (4, including header which has 3)
  EXPECT_EQ(table->column_count, 4u);
  ...
  // Verify column_count is recalculated (max is now 3 from header)
  EXPECT_EQ(table->column_count, 3u);

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, RowRemoveAllRowsSameLength) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Add rows all with same field count
  const char * row1[] = {"a", "b", "c"};
  const char * row2[] = {"d", "e", "f"};
  const char * row3[] = {"g", "h", "i"};
  ...
  // Remove one row (all have same length, so max unchanged)
  status = gtext_csv_row_remove(table, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify column_count unchanged (still 3)
  EXPECT_EQ(table->column_count, 3u);

  gtext_csv_free_table(table);
}

TEST(CsvIrregularRows, RowRemoveEmptyTableAfterRemoval) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
...
// ============================================================================
// Column Insertion Padding - Validation Tests
// ============================================================================

TEST(CsvColumnInsertionPadding, ColumnInsertStrictModeFailsBeyondRowLength) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Create table with irregular rows (but strict mode is default)
  const char * row1[] = {"a", "b", "c"}; // 3 columns
  const char * row2[] = {"d", "e"};      // 2 columns
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // In strict mode, we can't append irregular row, so create with
  // allow_irregular_rows then disable it
  gtext_csv_set_allow_irregular_rows(table, true);
  status = gtext_csv_row_append(table, row2, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  gtext_csv_set_allow_irregular_rows(table, false);

  // Try to insert column at index 3 (beyond row2's field_count of 2)
...
TEST(
    CsvColumnInsertionPadding, ColumnInsertIrregularModeAllowsBeyondRowLength) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Create table with irregular rows
  const char * row1[] = {"a", "b", "c"}; // 3 columns
  const char * row2[] = {"d", "e"};      // 2 columns
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Try to insert column at index 3 (beyond row2's field_count of 2)
  // Should succeed in irregular mode (padding will happen)
  status = gtext_csv_column_insert(table, 3, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvColumnInsertionPadding, ColumnInsertAtIndex0NoPaddingNeeded) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Insert at index 0 - all rows have at least 1 field, so no padding needed
...
TEST(CsvColumnInsertionPadding, ColumnInsertBeyondMaxColumnCount) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  gtext_csv_set_allow_irregular_rows(table, true);

  // Create table with rows of different lengths
  const char * row1[] = {"a", "b"}; // 2 columns
  const char * row2[] = {"c"};      // 1 column
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  status = gtext_csv_row_append(table, row2, nullptr, 1, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // column_count should be 2 (max of row lengths)
  EXPECT_EQ(table->column_count, 2u);

  // Insert at index 3 (beyond max column_count of 2)
  // Should succeed in irregular mode
  status = gtext_csv_column_insert(table, 3, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // column_count should be updated to 4
  EXPECT_EQ(table->column_count, 4u);

  gtext_csv_free_table(table);
}

TEST(CsvColumnInsertionPadding, ColumnInsertStrictModeFailsBeyondColumnCount) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Create regular table (strict mode)
  const char * row1[] = {"a", "b"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Try to insert beyond column_count
  status = gtext_csv_column_insert(table, 3, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
...
TEST(CsvColumnInsertionPadding,
    ColumnInsertIrregularModeAllowsBeyondColumnCount) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Insert beyond column_count - should succeed in irregular mode
  status = gtext_csv_column_insert(table, 3, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // column_count should be updated
  EXPECT_EQ(table->column_count, 4u);

  gtext_csv_free_table(table);
}

TEST(CsvColumnInsertionPadding, ColumnInsertWithHeadersStrictMode) {
  const char * headers[] = {"col1", "col2"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 2);
  ASSERT_NE(table, nullptr);

  // Add data row with fewer fields than header
  gtext_csv_set_allow_irregular_rows(table, true);
  const char * row1[] = {"a"}; // 1 field, but header has 2
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 1, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  gtext_csv_set_allow_irregular_rows(table, false);

  // Try to insert at index 2 (beyond data row's field_count of 1)
  // Should fail in strict mode
  status = gtext_csv_column_insert(table, 2, "newcol", 6);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);

  gtext_csv_free_table(table);
}

TEST(CsvColumnInsertionPadding, ColumnInsertWithHeadersIrregularMode) {
  const char * headers[] = {"col1", "col2"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 2);
  ASSERT_NE(table, nullptr);
  ...
  // Insert at index 2 (beyond data row's field_count of 1)
  // Should succeed in irregular mode
  status = gtext_csv_column_insert(table, 2, "newcol", 6);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
// ============================================================================
// Column Insertion Padding - Padding Verification Tests
// ============================================================================

TEST(CsvColumnInsertionPadding, ColumnInsertPadsSingleShortRow) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  gtext_csv_set_allow_irregular_rows(table, true);

  // Create table with one short row
  const char * row1[] = {"a", "b"}; // 2 columns
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Insert at index 3 (beyond row's field_count of 2)
  status = gtext_csv_column_insert(table, 3, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify row was padded: should have 4 fields now
  ...
  // Verify padding: fields at index 2 and 3 should be empty
  const char * field2 = gtext_csv_field(table, 0, 2, &len);
  ASSERT_NE(field2, nullptr);
  EXPECT_EQ(len, 0u);
  EXPECT_STREQ(field2, "");

  // Field at index 3 is the newly inserted empty field
  const char * field3 = gtext_csv_field(table, 0, 3, &len);
...
TEST(CsvColumnInsertionPadding, ColumnInsertPadsMultipleShortRows) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Insert at index 3 (beyond row2 and row3's field_count)
  status = gtext_csv_column_insert(table, 3, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify row1 (was long enough): should have 4 fields
  EXPECT_EQ(gtext_csv_col_count(table, 0), 4u);
  size_t len;
  const char * field = gtext_csv_field(table, 0, 3, &len);
  ASSERT_NE(field, nullptr);
  EXPECT_EQ(len, 0u); // New empty field

  // Verify row2 (was padded): should have 4 fields
  EXPECT_EQ(gtext_csv_col_count(table, 1), 4u);
  ...
  field = gtext_csv_field(table, 1, 2, &len);
  EXPECT_EQ(len, 0u);
  // New field at index 3
  field = gtext_csv_field(table, 1, 3, &len);
  EXPECT_EQ(len, 0u);

  // Verify row3 (was padded): should have 4 fields
  EXPECT_EQ(gtext_csv_col_count(table, 2), 4u);
  // Original field preserved
  field = gtext_csv_field(table, 2, 0, &len);
  EXPECT_STREQ(field, "f");
  // Padding at indices 1 and 2
  field = gtext_csv_field(table, 2, 1, &len);
  EXPECT_EQ(len, 0u);
  field = gtext_csv_field(table, 2, 2, &len);
  EXPECT_EQ(len, 0u);
...
TEST(CsvColumnInsertionPadding, ColumnInsertPadsAtVariousIndices) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Create table with one short row
  const char * row1[] = {"a"}; // 1 column
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 1, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Insert at index 2 (beyond row's field_count of 1)
  status = gtext_csv_column_insert(table, 2, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify padding: row should have 3 fields
  EXPECT_EQ(gtext_csv_col_count(table, 0), 3u);
  size_t len;
  const char * field0 = gtext_csv_field(table, 0, 0, &len);
  EXPECT_STREQ(field0, "a");
...
TEST(CsvColumnInsertionPadding, ColumnInsertWithValuesPadsCorrectly) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Insert at index 3 with values
  const char * values[] = {"val1", "val2"};
  size_t value_lengths[] = {4, 4};
  status = gtext_csv_column_insert_with_values(
      table, 3, nullptr, 0, values, value_lengths);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify row1 (was long enough): new field has value
  size_t len;
  const char * field = gtext_csv_field(table, 0, 3, &len);
  ASSERT_NE(field, nullptr);
  EXPECT_EQ(len, 4u);
  EXPECT_STREQ(field, "val1");

  // Verify row2 (was padded): padding + new field with value
  EXPECT_EQ(gtext_csv_col_count(table, 1), 4u);
  // Original fields preserved
  field = gtext_csv_field(table, 1, 0, &len);
  EXPECT_STREQ(field, "d");
  ...
  field = gtext_csv_field(table, 1, 2, &len);
  EXPECT_EQ(len, 0u);
  // New field at index 3 has value
  field = gtext_csv_field(table, 1, 3, &len);
  EXPECT_EQ(len, 4u);
  EXPECT_STREQ(field, "val2");

  gtext_csv_free_table(table);
}

TEST(CsvColumnInsertionPadding, ColumnInsertWithHeadersPadsHeaderRow) {
  const char * headers[] = {"col1", "col2"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 2);
  ASSERT_NE(table, nullptr);
  ...
  // Insert at index 3 (beyond header row's field_count of 2)
  status = gtext_csv_column_insert(table, 3, "newcol", 6);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify header row was padded: should have 4 fields
  // Access header row directly (it's at index 0 internally)
  EXPECT_EQ(table->rows[0].field_count, 4u);
  size_t len;
  const char * field = table->rows[0].fields[0].data;
  EXPECT_EQ(table->rows[0].fields[0].length, 4u);
  EXPECT_STREQ(field, "col1");
  field = table->rows[0].fields[1].data;
  EXPECT_EQ(table->rows[0].fields[1].length, 4u);
  EXPECT_STREQ(field, "col2");
  // Padding at index 2
  field = table->rows[0].fields[2].data;
  EXPECT_EQ(table->rows[0].fields[2].length, 0u);
  // New header at index 3
  field = table->rows[0].fields[3].data;
  EXPECT_EQ(table->rows[0].fields[3].length, 6u);
  EXPECT_STREQ(field, "newcol");

  // Verify data row was padded: should have 4 fields
  // Data row is at user index 0, internal index 1
  EXPECT_EQ(gtext_csv_col_count(table, 0), 4u);
  field = gtext_csv_field(table, 0, 0, &len);
  EXPECT_STREQ(field, "a");
  // Padding at indices 1 and 2
  ...
  // Verify header map entry for new column
  size_t idx;
...
TEST(CsvColumnInsertionPadding, ColumnInsertEmptyTableCreatesFirstColumn) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  gtext_csv_set_allow_irregular_rows(table, true);

  // Insert at index 0 in empty table
  GTEXT_CSV_Status status = gtext_csv_column_insert(table, 0, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify column_count is 1
  EXPECT_EQ(table->column_count, 1u);

  gtext_csv_free_table(table);
}

TEST(CsvColumnInsertionPadding, ColumnInsertAllRowsSameLengthNoPadding) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  gtext_csv_set_allow_irregular_rows(table, true);

  // Create table with all rows same length
  const char * row1[] = {"a", "b", "c"};
  const char * row2[] = {"d", "e", "f"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Insert at index 1 (within all rows' field_count)
  status = gtext_csv_column_insert(table, 1, nullptr, 0);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Verify no padding needed: all rows have 4 fields
  EXPECT_EQ(gtext_csv_col_count(table, 0), 4u);
  EXPECT_EQ(gtext_csv_col_count(table, 1), 4u);
...
// Normalization Tests
TEST(CsvNormalization, NormalizeToMaxBasic) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Add rows with different lengths
  const char * row1[] = {"a", "b"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 2, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Normalize to max
  status = gtext_csv_normalize_to_max(table);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // All rows should have 4 columns now
  EXPECT_EQ(gtext_csv_col_count(table, 0), 4u);
  EXPECT_EQ(gtext_csv_col_count(table, 1), 4u);
  EXPECT_EQ(gtext_csv_col_count(table, 2), 4u);
  EXPECT_EQ(table->column_count, 4u);
  ...
  // Row 3 should have one padded field
  field = gtext_csv_field(table, 2, 0, &len);
  EXPECT_STREQ(field, "g");
  field = gtext_csv_field(table, 2, 2, &len);
  EXPECT_STREQ(field, "i");
  field = gtext_csv_field(table, 2, 3, &len);
  EXPECT_EQ(len, 0u);
...
TEST(CsvNormalization, NormalizeToMaxAlreadyNormalized) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Add rows with same length
  const char * row1[] = {"a", "b", "c"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Normalize to max (should be no-op)
  status = gtext_csv_normalize_to_max(table);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Rows should still have 3 columns
  EXPECT_EQ(gtext_csv_col_count(table, 0), 3u);
  EXPECT_EQ(gtext_csv_col_count(table, 1), 3u);
...
TEST(CsvNormalization, NormalizeToSpecificCount) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Normalize to 3 columns (row 1 needs padding, row 2 is already 3)
  status = gtext_csv_normalize_rows(table, 3, false);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Row 1 should have one padded field
  size_t len;
  const char * field = gtext_csv_field(table, 0, 0, &len);
  EXPECT_STREQ(field, "a");
  field = gtext_csv_field(table, 0, 1, &len);
...
TEST(CsvNormalization, NormalizeToSpecificCountWithTruncation) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  const char * row2[] = {"c", "d", "e", "f", "g"};
  status = gtext_csv_row_append(table, row2, nullptr, 5, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Normalize to 3 columns with truncation enabled
  status = gtext_csv_normalize_rows(table, 3, true);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Row 2 should be truncated (last 2 fields removed)
  size_t len;
  const char * field = gtext_csv_field(table, 1, 0, &len);
  EXPECT_STREQ(field, "c");
  field = gtext_csv_field(table, 1, 2, &len);
...
TEST(CsvNormalization, NormalizeToSpecificCountWithoutTruncationFails) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Normalize to 3 columns without truncation should fail (row 2 is too long)
  status = gtext_csv_normalize_rows(table, 3, false);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);

  // Table should be unchanged (atomicity)
  EXPECT_EQ(gtext_csv_col_count(table, 0), 2u);
  EXPECT_EQ(gtext_csv_col_count(table, 1), 5u);
...
TEST(CsvNormalization, NormalizeToMin) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 4, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Normalize to min (SIZE_MAX means min)
  // Need truncation enabled to truncate longer rows to minimum
  status = gtext_csv_normalize_rows(table, SIZE_MAX, true);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // All rows should have 2 columns (minimum)
  EXPECT_EQ(gtext_csv_col_count(table, 0), 2u);
  EXPECT_EQ(gtext_csv_col_count(table, 1), 2u);
  EXPECT_EQ(gtext_csv_col_count(table, 2), 2u);
  EXPECT_EQ(table->column_count, 2u);

  // Row 1 should be truncated
  size_t len;
  const char * field = gtext_csv_field(table, 0, 0, &len);
  EXPECT_STREQ(field, "a");
  field = gtext_csv_field(table, 0, 1, &len);
...
TEST(CsvNormalization, NormalizeEmptyTable) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Normalize empty table
  GTEXT_CSV_Status status = gtext_csv_normalize_to_max(table);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  // Normalize to specific count
  status = gtext_csv_normalize_rows(table, 5, false);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvNormalization, NormalizeWithHeaders) {
  const char * headers[] = {"col1", "col2"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 2);
  ASSERT_NE(table, nullptr);
  ...
  // Header row should be normalized too (access via internal structure)
  EXPECT_EQ(table->rows[0].field_count, 3u);
  // Verify header row fields directly
  EXPECT_STREQ(table->rows[0].fields[0].data, "col1");
  EXPECT_STREQ(table->rows[0].fields[1].data, "col2");
  EXPECT_EQ(table->rows[0].fields[2].length, 0u);

  // Data rows should be normalized
  EXPECT_EQ(gtext_csv_col_count(table, 0), 3u);
  EXPECT_EQ(gtext_csv_col_count(table, 1), 3u);
  EXPECT_EQ(table->column_count, 3u);
...
TEST(CsvNormalization, NormalizeNullTable) {
  GTEXT_CSV_Status status = gtext_csv_normalize_to_max(nullptr);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);

  status = gtext_csv_normalize_rows(nullptr, 0, false);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
}

TEST(CsvNormalization, NormalizeAtomicity) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  size_t orig_col_count_0 = gtext_csv_col_count(table, 0);
  size_t orig_col_count_1 = gtext_csv_col_count(table, 1);

  // Try to normalize to 1 column without truncation (should fail)
  status = gtext_csv_normalize_rows(table, 1, false);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);

  // Table should be unchanged (atomicity)
  EXPECT_EQ(gtext_csv_col_count(table, 0), orig_col_count_0);
  EXPECT_EQ(gtext_csv_col_count(table, 1), orig_col_count_1);
...
TEST(CsvMutation, TableClearWithHeaders) {
  ...
  EXPECT_EQ(gtext_csv_row_count(table),
      0u);                         // No data rows (header excluded from count)
...
TEST(CsvMutation, TableClearCanAppendAfterClearing) {
  ...
  status = gtext_csv_row_append(table, new_row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  const char * new_row2[] = {"u", "v", "w"};
  status = gtext_csv_row_append(table, new_row2, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, TableCompactPreservesInSituFields) {
  ...
  // Note: Not all fields may be in-situ (depends on parsing), but some should
  // be
...
TEST(CsvMutation, CloneTableWithoutHeaders) {
  ...
  // Add rows
  const char * row1[] = {"a", "b", "c"};
  GTEXT_CSV_Status status = gtext_csv_row_append(source, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  const char * row2[] = {"d", "e", "f"};
  status = gtext_csv_row_append(source, row2, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, CloneIndependence) {
  ...
  status = gtext_csv_row_append(source, row3, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  for (size_t row = 0; row < clone->row_count; row++) {
    ...
    for (size_t col = 0; col < table_row->field_count; col++) {
      EXPECT_FALSE(table_row->fields[col].is_in_situ)
          << "Clone field at row " << row << ", col " << col
          << " should not be in-situ";
...
static void compare_tables(const GTEXT_CSV_Table * table1,
    const GTEXT_CSV_Table * table2, const char * context) {
  EXPECT_EQ(gtext_csv_row_count(table1), gtext_csv_row_count(table2))
      << "Row count mismatch: " << context;

  size_t min_rows =
      std::min(gtext_csv_row_count(table1), gtext_csv_row_count(table2));
  for (size_t row = 0; row < min_rows; row++) {
    EXPECT_EQ(
        gtext_csv_col_count(table1, row), gtext_csv_col_count(table2, row))
        << "Col count mismatch at row " << row << ": " << context;

    size_t min_cols = std::min(
        gtext_csv_col_count(table1, row), gtext_csv_col_count(table2, row));
    for (size_t col = 0; col < min_cols; col++) {
      size_t len1, len2;
      const char * field1 = gtext_csv_field(table1, row, col, &len1);
      ...
      if (len1 == len2 && len1 > 0) {
        EXPECT_EQ(memcmp(field1, field2, len1), 0)
            << "Field content mismatch at row " << row << ", col " << col
            << ": " << context;
      }
    }
  }
...
static void test_mutation_round_trip(const char * csv_data,
  ...
  // Parse original
  GTEXT_CSV_Table * original =
      gtext_csv_parse_table(csv_data, strlen(csv_data), parse_opts, &err);
  ...
  ASSERT_NE(before_mutation, nullptr)
      << test_name << ": Failed to clone before mutation";
...
TEST(CsvIntegration, RoundTripAppendRow) {
  ...
  test_mutation_round_trip(
      csv_data, &parse_opts,
      [](GTEXT_CSV_Table * table) {
        const char * new_row[] = {"g", "h", "i"};
        GTEXT_CSV_Status status =
            gtext_csv_row_append(table, new_row, nullptr, 3, nullptr);
        EXPECT_EQ(status, GTEXT_CSV_OK);
      },
      "RoundTripAppendRow");
}
...
TEST(CsvIntegration, RoundTripRemoveRow) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        GTEXT_CSV_Status status =
            gtext_csv_row_remove(table, 1); // Remove middle row
        EXPECT_EQ(status, GTEXT_CSV_OK);
      },
      "RoundTripRemoveRow");
}
...
TEST(CsvIntegration, RoundTripInsertRow) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        const char * new_row[] = {"x", "y", "z"};
        GTEXT_CSV_Status status =
            gtext_csv_row_insert(table, 1, new_row, nullptr, 3, nullptr);
        EXPECT_EQ(status, GTEXT_CSV_OK);
      },
      "RoundTripInsertRow");
}
...
TEST(CsvIntegration, RoundTripSetRow) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        const char * new_row[] = {"x", "y", "z"};
        GTEXT_CSV_Status status =
            gtext_csv_row_set(table, 1, new_row, nullptr, 3, nullptr);
        EXPECT_EQ(status, GTEXT_CSV_OK);
      },
      "RoundTripSetRow");
}
...
TEST(CsvIntegration, RoundTripSetField) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        GTEXT_CSV_Status status =
            gtext_csv_field_set(table, 1, 1, "modified", 8);
        EXPECT_EQ(status, GTEXT_CSV_OK);
      },
      "RoundTripSetField");
}
...
TEST(CsvIntegration, RoundTripRenameColumn) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        GTEXT_CSV_Status status =
            gtext_csv_column_rename(table, 0, "full_name", 0);
        EXPECT_EQ(status, GTEXT_CSV_OK);
      },
...
TEST(CsvIntegration, RoundTripMultipleMutations) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        ...
        // Note: Removed column_append as it changes column count and makes
        // round-trip comparison complex
...
TEST(CsvIntegration, RoundTripColumnAndRowOperations) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        ...
        const char * new_row[] = {"x", "y", "z", "w"};
        status = gtext_csv_row_insert(table, 1, new_row, nullptr, 4, nullptr);
        EXPECT_EQ(status, GTEXT_CSV_OK);
      },
...
TEST(CsvIntegration, RoundTripWithHeaders) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        ...
        status = gtext_csv_row_append(table, new_row, nullptr, 4, nullptr);
        EXPECT_EQ(status, GTEXT_CSV_OK);
      },
...
TEST(CsvIntegration, CloneMutateIndependence) {
  ...
  // Mutate original
  const char * new_row[] = {"x", "y", "z"};
  GTEXT_CSV_Status status = gtext_csv_row_append(original, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  EXPECT_NE(output1_len, output2_len)
      << "Clone and original should produce different output";
...
TEST(CsvIntegration, RoundTripWithTSVDialect) {
  // Test mutation on TSV table (round-trip write may have issues, so just test
  // mutation)
  ...
  // Parse original
  GTEXT_CSV_Table * original =
      gtext_csv_parse_table(tsv_data, strlen(tsv_data), &parse_opts, &err);
...
TEST(CsvIntegration, RoundTripComplexSequence) {
  ...
  test_mutation_round_trip(
      ...
      [](GTEXT_CSV_Table * table) {
        ...
        status = gtext_csv_row_append(table, new_row, nullptr, 5, nullptr);
        EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvIntegration, RoundTripEmptyFields) {
  // Test mutation on table with empty fields - create table manually to ensure
  // consistent columns
  GTEXT_CSV_Table * original = gtext_csv_new_table();
  ASSERT_NE(original, nullptr);

  const char * row1[] = {"a", "", "c"};
  GTEXT_CSV_Status status = gtext_csv_row_append(original, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  const char * row2[] = {"", "b", ""};
  status = gtext_csv_row_append(original, row2, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);

  const char * row3[] = {"", "", ""};
  status = gtext_csv_row_append(original, row3, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  status = gtext_csv_row_append(original, new_row, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, FieldSetDataCopied) {
  ...
  const char * field = gtext_csv_field(table, 0, 0, &len);
  EXPECT_STREQ(
      field, "test_data"); // Should still be "test_data", not "Xest_data"
...
TEST(CsvMutation, ColumnAppendAllRowsGetNewField) {
  ...
  for (size_t i = 0; i < 5; i++) {
    ...
    GTEXT_CSV_Status status = gtext_csv_row_append(table, row, nullptr, 1, nullptr);
    EXPECT_EQ(status, GTEXT_CSV_OK);
  }
...
TEST(CsvMutation, NewTableWithHeadersNullHeadersArray) {
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(nullptr, nullptr, 1);
  EXPECT_EQ(table, nullptr);
}
...
TEST(CsvMutation, NewTableWithHeadersZeroCount) {
  const char * headers[] = {"col1"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 0);
  EXPECT_EQ(table, nullptr);
}
...
TEST(CsvMutation, NewTableWithHeadersExplicitLengths) {
  ...
  size_t lengths[] = {4, 4};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, lengths, 2);
  ASSERT_NE(table, nullptr);
...
TEST(CsvMutation, ColumnAppendDuplicateHeaderNameAllowedByDefault) {
  ...
  // Verify header_index returns a match (may be the newly added one since it's
  // added to front of hash chain)
  size_t idx;
  status = gtext_csv_header_index(table, "col1", &idx);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  // The returned index may be 0 (original) or 2 (newly added), depending on
  // hash chain order
...
TEST(CsvMutation, ColumnInsertHeaderMapEntriesReindexed) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 4);
  ASSERT_NE(table, nullptr);
...
TEST(CsvMutation, ColumnInsertDuplicateHeaderNameAllowedByDefault) {
  ...
  // Verify header_index returns a match (may be the newly inserted one since
  // it's added to front of hash chain)
  size_t idx;
  status = gtext_csv_header_index(table, "col1", &idx);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  // The returned index may be 0 (original) or 1 (newly inserted), depending on
  // hash chain order
...
TEST(CsvMutation, ColumnAppendWithValuesBasic) {
  ...
  // Add a row
  const char * fields[] = {"a", "b"};
  EXPECT_EQ(gtext_csv_row_append(table, fields, nullptr, 2, nullptr), GTEXT_CSV_OK);

  // Append column with values
  const char * values[] = {"c", nullptr}; // One value for one data row
  EXPECT_EQ(
      gtext_csv_column_append_with_values(table, nullptr, 0, values, nullptr),
      GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnAppendWithValuesEmptyColumn) {
  ...
  EXPECT_EQ(
      gtext_csv_column_append_with_values(table, "col2", 0, nullptr, nullptr),
      GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnAppendWithValuesEmptyColumnNoHeaders) {
  ...
  EXPECT_EQ(gtext_csv_column_append_with_values(
                table, "ignored", 0, nullptr, nullptr),
      GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnAppendWithValuesValueCountMismatch) {
  ...
  EXPECT_EQ(gtext_csv_row_append(table, fields1, nullptr, 1, nullptr), GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_append(table, fields2, nullptr, 1, nullptr), GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnAppendWithValuesBinaryData) {
  ...
  const char binary_data[] = {
      'b', 'i', 'n', '\0', 'a', 'r', 'y'}; // Contains null byte
  const char * values[] = {binary_data, nullptr};
  size_t lengths[] = {7, 0};
  EXPECT_EQ(
      gtext_csv_column_append_with_values(table, nullptr, 0, values, lengths),
      GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnAppendWithValuesHeaderMapConsistency) {
  ...
  // Verify that header lookup works with the value from values[0] (not
  // header_name) This confirms header map entry points to the correct data
  EXPECT_EQ(gtext_csv_header_index(table, "col2", &idx), GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnInsertWithValuesBasic) {
  ...
  const char * values[] = {"c", nullptr};
  EXPECT_EQ(gtext_csv_column_insert_with_values(
                table, 1, nullptr, 0, values, nullptr),
      GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnInsertWithValuesInvalidIndex) {
  ...
  const char * values[] = {"b", nullptr};
  EXPECT_EQ(gtext_csv_column_insert_with_values(
                table, 2, nullptr, 0, values, nullptr),
      GTEXT_CSV_E_INVALID);
...
TEST(CsvMutation, ColumnAppendWithValuesMultipleRows) {
  ...
  EXPECT_EQ(gtext_csv_row_append(table, fields1, nullptr, 1, nullptr), GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_append(table, fields2, nullptr, 1, nullptr), GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_append(table, fields3, nullptr, 1, nullptr), GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnInsertWithValuesMultipleRows) {
  ...
  EXPECT_EQ(gtext_csv_row_append(table, fields1, nullptr, 2, nullptr), GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_append(table, fields2, nullptr, 2, nullptr), GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnRemoveHeaderMapEntriesReindexed) {
  ...
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 5);
  ASSERT_NE(table, nullptr);
...
TEST(CsvMutation, ColumnRemoveMultipleColumns) {
  ...
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 5, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvMutation, ColumnRenameInTableWithHeaders) {
  ...
  // Verify header field is updated in header row (access via internal
  // structure)
...
TEST(CsvMutation, ColumnRenameDuplicateHeaderNameAllowedByDefault) {
  ...
  // Verify header_index returns a match (may be the renamed one since it's
  // added to front of hash chain)
  size_t idx;
  status = gtext_csv_header_index(table, "col1", &idx);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  // The returned index may be 0 (original) or 1 (renamed), depending on hash
  // chain order
  EXPECT_TRUE(idx == 0u || idx == 1u);
...
TEST(CsvMutation, ColumnRenameEmptyName) {
  ...
  // Verify header map lookup fails for empty name (or works if empty names are
  // allowed) Note: Empty names might be allowed, but lookup might fail
  size_t idx;
  status = gtext_csv_header_index(table, "", &idx);
  // This might succeed or fail depending on implementation - both are
  // acceptable
...
TEST(CsvMutation, ColumnRenameRenameToSameName) {
  ...
  // This should work - it's not a duplicate because we exclude the column being
  // renamed
...
TEST(CsvMutation, CanHaveUniqueHeaders) {
  ...
  GTEXT_CSV_Table * table_duplicate =
      gtext_csv_parse_table(input, strlen(input), &opts, nullptr);
...
TEST(CsvMutation, SetHeaderRowEnable) {
  ...
  // Add rows
  EXPECT_EQ(gtext_csv_row_append(table, fields1, nullptr, 3, nullptr), GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_append(table, fields2, nullptr, 3, nullptr), GTEXT_CSV_OK);
  EXPECT_EQ(gtext_csv_row_append(table, fields3, nullptr, 3, nullptr), GTEXT_CSV_OK);
...
TEST(CsvMutation, SetHeaderRowEnableColumnCountHandling) {
  ...
  // Enable headers - first row has 2 columns, column_count is 2, so no change
  // needed
  GTEXT_CSV_Status status = gtext_csv_set_header_row(table, true);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 2u);
...
TEST(CsvMutation, SetHeaderRowEnableColumnCountUpdate) {
  ...
  // Now enable headers - first row has 3 columns, column_count is 3, so no
  // change
  GTEXT_CSV_Status status = gtext_csv_set_header_row(table, true);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 3u);
...
TEST(CsvMutation, SetHeaderRowEnableWithUniqueHeadersRequired) {
  // Test enabling headers when require_unique_headers is true and headers are
  // unique
  const char * fields1[] = {"name", "age", "city"};
  const char * fields2[] = {"John", "30", "NYC"};
...
TEST(CsvMutation, SetHeaderRowEnableWithUniqueHeadersRequiredDuplicate) {
  // Test enabling headers when require_unique_headers is true and headers have
  // duplicates
...
TEST(CsvMutation, SetHeaderRowEnableWithUniqueHeadersNotRequiredDuplicate) {
  // Test enabling headers when require_unique_headers is false and headers have
  // duplicates
  const char * fields1[] = {"name", "name", "age"}; // Duplicate "name"
  const char * fields2[] = {"John", "Jane", "30"};
...
TEST(CsvMutation, SetHeaderRowRecoverabilityDuplicateHeaders) {
  // Test recoverability: table with duplicate headers, disable headers, fix
  // duplicates, re-enable
  const char * fields1[] = {"name", "name", "age"}; // Duplicate "name"
  const char * fields2[] = {"John", "Jane", "30"};
  ...
  // Note: When duplicates are allowed, only the first occurrence is in the
  // header map, but we can still rename by column index
  EXPECT_EQ(gtext_csv_set_require_unique_headers(table, true), GTEXT_CSV_OK);
...
TEST(CsvMutation, SetHeaderRowRecoverabilityEnableAfterDisable) {
  // Test recoverability: table with headers, try to enable headers again
  // (error), disable first, then enable
  const char * headers[] = {"col1", "col2"};
  GTEXT_CSV_Table * table =
      gtext_csv_new_table_with_headers(headers, nullptr, 2);
  ASSERT_NE(table, nullptr);
...
TEST(CsvMutation, SetHeaderRowRecoverabilityUniqueHeadersRequired) {
  // Test recoverability: table with require_unique_headers=true, headers have
  // duplicates, disable, fix, re-enable
  const char * fields1[] = {"name", "name", "age"}; // Duplicate "name"
  const char * fields2[] = {"John", "Jane", "30"};
  ...
  // Fix duplicates by setting the field value in the first row (which will
  // become header) Change column 1 from "name" to "name2"
  EXPECT_EQ(gtext_csv_field_set(table, 0, 1, "name2", 5), GTEXT_CSV_OK);
...
TEST(CsvMutation, FieldSetWithHeader) {
  ...
  // Set field in data row (row index 0 is first data row, header is at internal
  // index 0)
...
TEST(CsvValidation, HasIrregularRowsRegularTable) {
  // Create a regular table (all rows same length)
  const char * csv_data = "col1,col2,col3\nval1,val2,val3\nval4,val5,val6\n";
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  opts.dialect.treat_first_row_as_header = true;
  ...
  // Regular table should not have irregular rows
  EXPECT_FALSE(gtext_csv_has_irregular_rows(table));

  gtext_csv_free_table(table);
}

TEST(CsvValidation, HasIrregularRowsIrregularTable) {
  ...
  const char * fields2[] = {"c", "d", "e"};
  EXPECT_EQ(gtext_csv_row_append(table, fields2, nullptr, 3, nullptr), GTEXT_CSV_OK);

  // Table should have irregular rows
  EXPECT_TRUE(gtext_csv_has_irregular_rows(table));

  gtext_csv_free_table(table);
}

TEST(CsvValidation, HasIrregularRowsEmptyTable) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Empty table should not have irregular rows
  EXPECT_FALSE(gtext_csv_has_irregular_rows(table));

  gtext_csv_free_table(table);
}

TEST(CsvValidation, HasIrregularRowsSingleRow) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Single row table should not have irregular rows
  EXPECT_FALSE(gtext_csv_has_irregular_rows(table));

  gtext_csv_free_table(table);
}

TEST(CsvValidation, HasIrregularRowsNullTable) {
  // NULL table should return false
  EXPECT_FALSE(gtext_csv_has_irregular_rows(nullptr));
}

TEST(CsvValidation, MaxColCountRegularTable) {
  const char * csv_data = "col1,col2,col3\nval1,val2,val3\n";
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  opts.dialect.treat_first_row_as_header = true;
  ...
  EXPECT_EQ(gtext_csv_max_col_count(table), 3u);

  gtext_csv_free_table(table);
}

TEST(CsvValidation, MaxColCountIrregularTable) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  const char * fields2[] = {"c", "d", "e", "f"};
  EXPECT_EQ(gtext_csv_row_append(table, fields2, nullptr, 4, nullptr), GTEXT_CSV_OK);

  const char * fields3[] = {"g", "h", "i"};
  EXPECT_EQ(gtext_csv_row_append(table, fields3, nullptr, 3, nullptr), GTEXT_CSV_OK);

  EXPECT_EQ(gtext_csv_max_col_count(table), 4u);

  gtext_csv_free_table(table);
}

TEST(CsvValidation, MaxColCountEmptyTable) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  EXPECT_EQ(gtext_csv_max_col_count(table), 0u);

  gtext_csv_free_table(table);
}

TEST(CsvValidation, MaxColCountNullTable) {
  EXPECT_EQ(gtext_csv_max_col_count(nullptr), 0u);
}

TEST(CsvValidation, MinColCountRegularTable) {
  const char * csv_data = "col1,col2,col3\nval1,val2,val3\n";
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  opts.dialect.treat_first_row_as_header = true;
  ...
  EXPECT_EQ(gtext_csv_min_col_count(table), 3u);

  gtext_csv_free_table(table);
}

TEST(CsvValidation, MinColCountIrregularTable) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  EXPECT_EQ(gtext_csv_set_allow_irregular_rows(table, true), GTEXT_CSV_OK);

  const char * fields1[] = {"a", "b", "c", "d"};
  EXPECT_EQ(gtext_csv_row_append(table, fields1, nullptr, 4, nullptr), GTEXT_CSV_OK);

  const char * fields2[] = {"e", "f"};
  EXPECT_EQ(gtext_csv_row_append(table, fields2, nullptr, 2, nullptr), GTEXT_CSV_OK);

  const char * fields3[] = {"g", "h", "i"};
  EXPECT_EQ(gtext_csv_row_append(table, fields3, nullptr, 3, nullptr), GTEXT_CSV_OK);

  EXPECT_EQ(gtext_csv_min_col_count(table), 2u);

  gtext_csv_free_table(table);
}

TEST(CsvValidation, MinColCountEmptyTable) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  EXPECT_EQ(gtext_csv_min_col_count(table), 0u);

  gtext_csv_free_table(table);
}

TEST(CsvValidation, MinColCountNullTable) {
  EXPECT_EQ(gtext_csv_min_col_count(nullptr), 0u);
}

TEST(CsvValidation, ValidateTableValidRegular) {
  const char * csv_data = "col1,col2,col3\nval1,val2,val3\n";
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  opts.dialect.treat_first_row_as_header = true;
  ...
  EXPECT_EQ(gtext_csv_validate_table(table), GTEXT_CSV_OK);

  gtext_csv_free_table(table);
}

TEST(CsvValidation, ValidateTableValidIrregular) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
...
TEST(CsvValidation, ValidateTableEmpty) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
...
TEST(CsvValidation, ValidateTableNull) {
  EXPECT_EQ(gtext_csv_validate_table(nullptr), GTEXT_CSV_E_INVALID);
}

TEST(CsvValidation, ValidateTableWithHeaders) {
  const char * csv_data = "col1,col2,col3\nval1,val2,val3\n";
  GTEXT_CSV_Parse_Options opts = gtext_csv_parse_options_default();
  opts.dialect.treat_first_row_as_header = true;
...
TEST(CsvErrorMessages, RowAppendFieldCountMismatch) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Add first row with 3 fields
  const char * row1[] = {"a", "b", "c"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  EXPECT_EQ(table->column_count, 3u);

  // Try to append row with wrong field count (2 fields instead of 3)
  const char * row2[] = {"d", "e"};
  GTEXT_CSV_Error err{};
  ...
  EXPECT_NE(strstr(err.message, "expected"), nullptr);
  EXPECT_NE(strstr(err.message, "got"), nullptr);
  EXPECT_NE(strstr(err.message, "3"), nullptr); // Expected count
  EXPECT_NE(strstr(err.message, "2"), nullptr); // Actual count
  EXPECT_EQ(err.row_index, SIZE_MAX); // Append operation uses SIZE_MAX
  EXPECT_EQ(err.col_index, 2u); // Actual count stored in col_index

  gtext_csv_error_free(&err);
  gtext_csv_free_table(table);
}

TEST(CsvErrorMessages, RowInsertFieldCountMismatch) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Try to insert row with wrong field count at index 0
  const char * row2[] = {"d", "e"};
  GTEXT_CSV_Error err{};
  status = gtext_csv_row_insert(table, 0, row2, nullptr, 2, &err);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
  EXPECT_EQ(err.code, GTEXT_CSV_E_INVALID);
  EXPECT_NE(err.message, nullptr);
  EXPECT_NE(strstr(err.message, "expected"), nullptr);
  EXPECT_NE(strstr(err.message, "got"), nullptr);
  EXPECT_NE(strstr(err.message, "row 0"), nullptr); // Row index in message
  EXPECT_EQ(err.row_index, 0u); // Row index
  EXPECT_EQ(err.col_index, 2u); // Actual count

  gtext_csv_error_free(&err);
  gtext_csv_free_table(table);
}

TEST(CsvErrorMessages, RowSetFieldCountMismatch) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);

  // Add rows with 3 fields
  const char * row1[] = {"a", "b", "c"};
  const char * row2[] = {"d", "e", "f"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
  ...
  // Try to replace row 0 with wrong field count (2 fields instead of 3)
  const char * new_row[] = {"x", "y"};
  GTEXT_CSV_Error err{};
  status = gtext_csv_row_set(table, 0, new_row, nullptr, 2, &err);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
  EXPECT_EQ(err.code, GTEXT_CSV_E_INVALID);
  EXPECT_NE(err.message, nullptr);
  EXPECT_NE(strstr(err.message, "expected"), nullptr);
...
TEST(CsvErrorMessages, RowAppendFieldCountMismatchWithHeaders) {
  const char * headers[] = {"col1", "col2", "col3"};
  GTEXT_CSV_Table * table = gtext_csv_new_table_with_headers(headers, nullptr, 3);
  ASSERT_NE(table, nullptr);

  // Add a data row with correct field count
  const char * row1[] = {"a", "b", "c"};
  GTEXT_CSV_Status status = gtext_csv_row_append(table, row1, nullptr, 3, nullptr);
  EXPECT_EQ(status, GTEXT_CSV_OK);
...
TEST(CsvErrorMessages, ErrorMessageNullSafety) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Try to append with wrong field count, but pass NULL for error
...
TEST(CsvErrorMessages, ErrorMessageMemoryManagement) {
  GTEXT_CSV_Table * table = gtext_csv_new_table();
  ASSERT_NE(table, nullptr);
  ...
  // Try to append with wrong field count
  const char * row2[] = {"d", "e"};
  GTEXT_CSV_Error err{};
  status = gtext_csv_row_append(table, row2, nullptr, 2, &err);
  EXPECT_EQ(status, GTEXT_CSV_E_INVALID);
