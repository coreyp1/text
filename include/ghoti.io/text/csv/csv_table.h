 ...
 */
typedef struct GTEXT_CSV_Table GTEXT_CSV_Table;
 ...
 * - Fields that don't require transformation (no unescaping, no UTF-8
 * validation) may reference the input buffer directly for zero-copy efficiency.
 ...
 * - When `validate_utf8` is enabled, all fields are copied (in-situ mode is
 * disabled).
 *
 * @param data Input CSV data (must remain valid for table lifetime if
 * in_situ_mode is enabled)
 ...
 * @return Number of rows (0-based count of data rows, excluding header if
 * present)
 */
 ...
 * row is not accessible via this function. Row indices are 0-based for data
 * rows.
 *
 * @param table Table (must not be NULL)
 ...
 */
GTEXT_API size_t gtext_csv_col_count(const GTEXT_CSV_Table * table, size_t row);

/**
 * @brief Get a field value from the table
 *
 * Returns a pointer to the field data. The pointer is valid for the lifetime
 * of the table. If in-situ mode was used, the pointer may reference the
 * original input buffer (which must remain valid). Otherwise, the pointer
 * references arena-allocated memory.
 *
 * The row index refers to data rows only. If the table has headers, the header
 * row is not accessible via this function. Row indices are 0-based for data
 * rows.
 ...
 * @return GTEXT_CSV_OK on success, GTEXT_CSV_E_INVALID if column not found
 */
 ...
 * This function is useful for iterating through all columns with duplicate
 * header names.
 *
 ...
 * 1. Call `gtext_csv_header_index()` to get the first match
 * 2. Repeatedly call `gtext_csv_header_index_next()` with the previous index
 *    until it returns `GTEXT_CSV_E_INVALID` (no more matches)
 *
 * Only works if the table was parsed with header processing enabled.
 *
 * @param table Table (must not be NULL)
 * @param name Column name to look up
 * @param current_idx Current column index (must be a valid index for this
 * header name)
 * @param out_idx Output parameter for the next column index (must not be NULL)
 * @return GTEXT_CSV_OK on success, GTEXT_CSV_E_INVALID if no more matches found
 ...
 *
 * The table must be freed with gtext_csv_free_table() when no longer needed.
 *
 ...
 * @param header_lengths Array of header name lengths, or NULL if all are
 * null-terminated
 * @param header_count Number of headers (must be > 0)
 * @return New table with headers, or NULL on allocation failure or duplicate
 * header names
 */
 ...
 *
 * All field data is copied to the arena and does not reference external
 * buffers. If field_lengths is NULL, all fields are assumed to be
 ...
 * The row index refers to data rows only. If the table has headers, the header
 * row is not accessible via this function. Row indices are 0-based for data
 * rows. The function internally adjusts the index to account for the header
 * row.
 *
 * **Atomic Operation**: This function is atomic - either the entire operation
 * succeeds and the table remains in a consistent state, or it fails and the
 * table remains unchanged. All memory allocations are performed before any
 * state changes (including row shifting), ensuring no partial state
 * modifications.
 *
 * @param table Table (must not be NULL)
 * @param row_idx Row index where to insert (0-based, data rows only, must be <=
 * row_count)
 * @param fields Array of field data pointers (must not be NULL)
 * @param field_lengths Array of field lengths, or NULL if all fields are
 * null-terminated
 ...
 * are 0-based for data rows. The function internally adjusts the index to
 ...
 * **Atomic Operation**: This function is atomic - either the entire operation
 * succeeds and the table remains in a consistent state, or it fails and the
 * table remains unchanged. This is a simple operation with no memory
 * allocations that can fail, ensuring atomicity.
 *
 * @param table Table (must not be NULL)
 ...
 * bulk-allocated in one contiguous block before any field structures are
 * updated, ensuring no partial state modifications.
 *
 * @param table Table (must not be NULL)
 ...
 * The field data is copied to the arena and does not reference external
 * buffers. If the field was previously in-situ (referencing the input buffer),
 * it will be copied to the arena.
 *
 ...
 * is inherently atomic - if allocation fails, the field structure is not
 * updated.
 *
 * @param table Table (must not be NULL)
 * @param row Row index (0-based, data rows only)
 ...
 *
 * This function is automatically called by `GTEXT_CSV_Table_clear()`, but can
 ...
 * atomic state update phase. If any allocation fails, no state has been
 * modified.
 *
 * @param table Table (must not be NULL)
 * @param header_name Header name for the new column (ignored if table has no
 * headers, can be NULL)
 * @param header_name_len Length of header name, or 0 if null-terminated
 * (ignored if table has no headers)
 * @return GTEXT_CSV_OK on success, error code on failure
 */
 ...
 * - If the table has headers: value count must match `row_count + 1` (data rows
 * + header row)
 * - If the table has no headers: value count must match `row_count`
 * - If the table is empty (`row_count == 0`), returns `GTEXT_CSV_E_INVALID`
 *
 * **Header Handling**:
 * - If the table has headers and `values` is provided:
 *   - `values[0]` is used for both the header field value and the header map
 * entry
 *   - The `header_name` parameter is ignored (can be NULL)
 * - If the table has headers and `values` is NULL (empty column):
 *   - `header_name` is used for both the header field value and the header map
 * entry
 *   - `header_name` must not be NULL
 * - If the table has no headers: `header_name` is ignored
 *
 * **Uniqueness Validation**:
 * - If `require_unique_headers` is `true`, validates that the header value
 * doesn't already exist
 * - The check occurs before any allocations or state changes
 * - If a duplicate is found, returns `GTEXT_CSV_E_INVALID` and leaves the table
 * unchanged
 *
 * **Value Length Handling**:
 * - If `value_lengths` is NULL, all values are treated as null-terminated
 * strings (uses strlen())
 * - If `value_lengths` is provided, individual entries with length 0 use
 * strlen() fallback
 *
 * All value data is copied to the arena and does not reference external
 * buffers.
 *
 * **Atomic Operation**: This function is atomic - either the entire operation
 * succeeds and the table remains in a consistent state, or it fails and the
 ...
 *
 * @param table Table (must not be NULL)
 * @param header_name Header name for the new column (used when values is NULL
 * and table has headers, ignored otherwise)
 * @param header_name_len Length of header name, or 0 if null-terminated
 * @param values Array of field values (NULL for empty column, must match row
 * count if provided)
 * @param value_lengths Array of value lengths, or NULL if all are
 * null-terminated
 * @return GTEXT_CSV_OK on success, error code on failure
 */
 ...
 * - All header map entries with index >= col_idx are reindexed (incremented by
 * 1)
 ...
 * final atomic state update. If any allocation fails, no state has been
 * modified.
 *
 * @param table Table (must not be NULL)
 * @param col_idx Column index where to insert (0-based, must be <= column
 * count)
 * @param header_name Header name for the new column (required if table has
 * headers, ignored otherwise)
 * @param header_name_len Length of header name, or 0 if null-terminated
 * (ignored if table has no headers)
 * @return GTEXT_CSV_OK on success, error code on failure
 */
 ...
 *
 * Removes the column at the specified index from all rows in the table,
 * shifting remaining columns left. The column index must be valid (must be <
 * column count).
 *
 ...
 * - All header map entries with index > col_idx are reindexed (decremented by
 * 1)
 *
 ...
 * and updating the header map, but no memory allocations that can fail,
 * ensuring atomicity. Header map entry removal and reindexing occur before the
 ...
 * if they would create duplicate header names. When set to `false` (the
 * default), duplicate header names are allowed.
 *
 ...
 *
 * @param table Table (must not be NULL)
 * @param require Whether to require unique headers (true) or allow duplicates
 * (false)
 ...
 *
 * Returns `true` if the table has headers and all header names are currently
 * unique. Returns `false` if:
 * - The table does not have headers
 * - The table has headers but contains duplicate header names
 * - The table is NULL
 *
 * This function is useful for checking if a table is in a state where unique
 * headers can be enforced (i.e., before enabling `require_unique_headers`).
 *
 * @param table Table (must not be NULL)
 ...
 * - If the table is empty, returns `GTEXT_CSV_E_INVALID`
 * - If headers already exist, returns `GTEXT_CSV_E_INVALID`
 * - If `require_unique_headers` is `true`, validates that all header names are
 * unique
 ...
 * - If headers don't exist, returns `GTEXT_CSV_E_INVALID`
 *
 * **Atomic Operation**: This function is atomic - either the entire operation
 * succeeds and the table remains in a consistent state, or it fails and the
 * table remains unchanged. All memory allocations are performed before any
 ...
 * buffers.
 *
 * The cloned table must be freed separately with gtext_csv_free_table()
